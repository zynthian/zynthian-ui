...
class zynthian_engine_jalv(zynthian_engine):
    ...
    plugins_dict = OrderedDict([
        ("Dexed", {'TYPE': "MIDI Synth",
         'URL': "https://github.com/dcoredump/dexed.lv2"}),
        ("Helm", {'TYPE': "MIDI Synth", 'URL': "http://tytel.org/helm"}),
        ("MDA ePiano", {'TYPE': "MIDI Synth",
                        'URL': "http://moddevices.com/plugins/mda/EPiano"}),
        ("MDA Piano", {'TYPE': "MIDI Synth",
                       'URL': "http://moddevices.com/plugins/mda/Piano"}),
        ("MDA JX10", {'TYPE': "MIDI Synth",
                      'URL': "http://moddevices.com/plugins/mda/JX10"}),
        ("MDA DX10", {'TYPE': "MIDI Synth",
                      'URL': "http://moddevices.com/plugins/mda/DX10"}),
        ("Obxd", {'TYPE': "MIDI Synth",
                  'URL': "https://obxd.wordpress.com"}),
        ("SynthV1", {'TYPE': "MIDI Synth",
                     'URL': "http://synthv1.sourceforge.net/lv2"}),
        ("Noize Mak3r", {
            'TYPE': "MIDI Synth", 'URL': "http://kunz.corrupt.ch/products/tal-noisemaker"}),
        ("Triceratops", {'TYPE': "MIDI Synth",
                         'URL': "http://nickbailey.co.nr/triceratops"}),
        ("Raffo MiniMoog", {'TYPE': "MIDI Synth",
                            'URL': "http://example.org/raffo"})
    ])
    ...
    def __init__(self, plugin_name, plugin_type, zyngui=None, dryrun=False):
        ...
        if not dryrun:
            if self.config_remote_display() and self.ui:
                self.command = (
                    "jalv.gtk --jack-name {} {}".format(self.get_jalv_jackname(), self.plugin_url))
            else:
                self.command_env['DISPLAY'] = "X"
                self.command = (
                    "jalv -n {} {}".format(self.get_jalv_jackname(), self.plugin_url))
            ...
            try:
                ...
                logging.info(
                    "No defined MIDI controllers for '{}'.".format(self.plugin_name))
        ...
        for bank_label, info in self.preset_info.items():
            self.bank_list.append(
                (str(info['bank_url']), None, bank_label, None))
    ...
    def get_jalv_jackname(self):
        try:
            jname = re.sub("[\_]{2,}", "_", re.sub(
                "[\'\*\(\)\[\]\s]", "_", self.plugin_name))
            jname_count = self.zyngui.screens['layer'].get_jackname_count(
                jname)
        except:
            jname_count = 0
            ...
            try:
                ...
                if len(ctrl_set) >= 4:
                    ...
                    self._ctrl_screens.append(
                        ["{}#{}".format(self.plugin_name, c), ctrl_set])
                    ctrl_set = []
                    c = c+1
