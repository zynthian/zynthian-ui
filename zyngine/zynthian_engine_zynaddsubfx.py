...
class zynthian_engine_zynaddsubfx(zynthian_engine):
    ...
    _ctrl_screens = [
        ...
        ['portamento', ['portamento on/off', 'portamento time',
                        'portamento up/down', 'portamento thresh']],
        ['modulation', ['modulation', 'modulation amplitude',
                        'modulation depth', 'modulation exp']],
        ['resonance', ['resonance center', 'res.center depth',
                       'resonance bandwidth', 'res.bw depth']],
        ['bandwidth', ['volume', 'bandwidth', 'bandwidth depth', 'bandwidth exp']],
        ['velocity', ['volume', 'panning', 'velocity sens.', 'velocity offs.']],
        ['depth', ['volume', 'panning depth',
                   'filter.cutoff depth', 'filter.Q depth']]
    ]
    ...
    def add_layer(self, layer):
        ...
        logging.debug("ADD LAYER => Part {} ({})".format(
            layer.part_i, self.jackname))
    ...
    def set_midi_chan(self, layer):
        if layer.part_i is not None:
            liblo.send(self.osc_target, "/part%d/Prcvchn" %
                       layer.part_i, layer.get_midi_chan())
    ...
    def enable_part(self, layer):
        if layer.part_i is not None:
            liblo.send(self.osc_target, "/part%d/Penabled" %
                       layer.part_i, True)
            liblo.send(self.osc_target, "/part%d/Prcvchn" %
                       layer.part_i, layer.get_midi_chan())
    ...
    def send_controller_value(self, zctrl):
        if zctrl.osc_path:
            liblo.send(self.osc_target, zctrl.osc_path,
    ...
    def XXX_midi_unlearn(self, zctrl):
        if zctrl.osc_path in self.slot_zctrls:
            ...
            try:
                ...
                liblo.send(self.osc_target, "/automate/slot%d/clear" %
                           zctrl.slot_i)
                self.osc_server.del_method(zctrl.osc_path, 'i')
                logging.info("Automate Slot %d Cleared => %s" %
                             (zctrl.slot_i, zctrl.osc_path))
                zctrl.slot_i = None
                return zctrl._unset_midi_learn()
            except Exception as e:
                logging.warning("Can't Clear Automate Slot %s => %s" %
                                (zctrl.osc_path, e))
    ...
    def XXX_set_midi_learn(self, zctrl, chan, cc):
        try:
            if zctrl.osc_path and zctrl.slot_i is not None and chan is not None and cc is not None:
                logging.info("Set Automate Slot %d: %s => %d, %d" %
                             (zctrl.slot_i, zctrl.osc_path, chan, cc))
            ...
            logging.error("Can't set slot automation for %s => %s" %
                          (zctrl.osc_path, e))
            return zctrl._unset_midi_learn()
    ...
    def XXX_cb_osc_automate_active_slot(self, path, args, types, src):
        if self.current_slot_zctrl:
            ...
            if self.current_slot_zctrl.midi_learn_cc is not None:
                zcc = (self.current_slot_zctrl.midi_learn_chan * 128) + \
                    self.current_slot_zctrl.midi_learn_cc
                liblo.send(self.osc_target,
                           "/automate/slot%d/learning" % slot_i, 0)
                liblo.send(self.osc_target, "/automate/slot%d/active" %
                           slot_i, True)
                # sleep(0.05)
                liblo.send(self.osc_target, "/automate/slot%d/name" %
                           slot_i, self.current_slot_zctrl.symbol)
                ...
                liblo.send(self.osc_target, "/automate/slot%d/param0/path" %
                           slot_i, self.current_slot_zctrl.osc_path)
    ...
    def XXX_cb_osc_param_change(self, path, args):
        if path in self.slot_zctrls:
            ...
            if zctrl.midi_learn_cc is None:
                liblo.send(self.osc_target,
                           "/automate/slot%d/midi-cc" % zctrl.slot_i)
    ...
    def XXX_cb_osc_automate_slot_midi_cc(self, path, args, types, src):
        ...
        if self.current_slot_zctrl and args[0] >= 0:
            try:
                ...
                cc = int(args[0]) % 128
                logging.debug("Automate Slot %d MIDI-CC: %s => %s" %
                              (slot_i, path, args[0]))
                ...
                logging.error(
                    "Can't match zctrl slot for the returned MIDI-CC! => %s" % e)
