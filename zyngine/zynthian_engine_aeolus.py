...
class zynthian_engine_aeolus(zynthian_engine):
            ...
            for line in self.config_lines:
                if line.startswith("/tuning"):
                    ...
                    try:
                        ...
                        logging.info("Current tuning frequency = {:.1f}".format(
                            self.current_tuning_freq))
                    except Exception as e:
                        logging.error(
                            "Can't get current tuning frequency! Using default (440.0 Hz) => {}".format(e))
                        self.current_tuning_freq = 440.0
                    try:
                        self.current_tuning_temp = int(parts[1])
                        logging.info("Current tuning temperament = {:d}".format(
                            self.current_tuning_temp))
                    except Exception as e:
                        logging.error(
                            "Can't get current tuning temperament! Using default (Equally Tempered) => {}".format(e))
    ...
    def fix_config(self):
        ...
        tuning_line = "/tuning {:.1f} {:d}\n".format(
            self.zyngui.fine_tuning_freq, self.tuning_temp)
        # Get current config ...
    ...
    def get_preset_list(self, bank):
        ...
        for gc in self.presets_data['group_config']:
            if gc['preset'] > i and gc['bank'] == bank[1]:
                ...
                res.append([str(bank[1]) + '/' + title,
                           [0, bank[1], i], title, gc['gconf']])
        return res

    def set_preset(self, layer, preset, preload=False):
        # Send Program Change
        self.zyngui.zynmidi.set_midi_preset(
            layer.get_midi_chan(), preset[1][0], preset[1][1], preset[1][2])
    ...
    def generate_ctrl_list(cls):
        ...
        for ig, group in enumerate(cls.instrument):
            ...
            for ctrl_name in group['buttons']:
                ...
                cls.instrument[ig]['ctrls'].append(
                    [ctrl_name, cc_num, 'off', 'off|on', [ig, i]])
                i += 1
                n += 1
            ...
            for ctrl in cls.instrument[ig]['ctrls']:
                ...
                if len(ctrl_set) == 4:
                    cls.instrument[ig]['ctrl_screens'].append(
                        ["{} ({})".format(group['name'], i), ctrl_set])
                    ctrl_set = []
                    i += 1
            if len(ctrl_set) > 0:
                cls.instrument[ig]['ctrl_screens'].append(
    ...
    def read_presets_file(cls):
        ...
        with open(cls.presets_fpath, mode='rb') as file:
            ...
            if n_groups != len(cls.instrument):
                logging.error("Number of groups ({}) doesn't fit with engine's configuration ({}) !".format(
                    n_groups, len(cls.instrument)))
            ...
            for num in range(8):
                ...
                for group in range(16):
                    ...
                    logging.debug(
                        "CHAN CONFIG (NUM {0}, GROUP {1} => {2:b}".format(num, group, res[0]))
            ...
            try:
                while True:
                    ...
                    if res[0] >= cls.n_banks:
                        logging.error("FORMAT => Bank index ({}>={})".format(
                            res[0], cls.n_banks))
                        return
                    if res[1] >= cls.n_presets:
                        logging.error("FORMAT => Preset index ({}>={})".format(
                            res[1], cls.n_presets))
                        return
                    logging.debug(
                        "BANK {}, PRESET {} =>".format(res[0], res[1]))
                    gconf = []
                    for group in range(n_groups):
                        ...
                        gconf.append(gc[0])
                        logging.debug(
                            "GROUP CONFIG {0} => {1:b}".format(group, gc[0]))
