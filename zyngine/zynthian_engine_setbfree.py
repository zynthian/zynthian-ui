...
class zynthian_engine_setbfree(zynthian_engine):
    ...
    bank_manuals_list = [
        ...
        ['Pedals + Lower + Upper', 3,
         'Pedals + Lower + Upper', '_', [True, True, 59]],
        ['Split: Lower + Upper', 4, 'Split Lower + Upper', '_', [True, False, 56]],
        ['Split: Pedals + Upper', 5, 'Split Pedals + Upper', '_', [False, True, 58]],
        ['Split: Pedals + Lower + Upper', 6,
         'Split Pedals + Lower + Upper', '_', [True, True, 57]]
    ]
    ...
    def __init__(self, zyngui=None):
        ...
        if self.config_remote_display():
            self.command = "setBfree -p \"{}\" -c \"{}\"".format(
                self.presets_fpath, self.config_autogen_fpath)
        else:
            self.command = "setBfree -p \"{}\" -c \"{}\"".format(
                self.presets_fpath, self.config_autogen_fpath)
    ...
    def generate_config_file(self, midi_chans):
        ...
        with open(self.config_tpl_fpath, 'r') as cfg_tpl_file:
            ...
            cfg_data = cfg_data.replace('#OSC.TUNING#', str(
                int(self.zyngui.fine_tuning_freq)))
            cfg_data = cfg_data.replace(
                '#MIDI.UPPER.CHANNEL#', str(1 + midi_chans[0]))
            cfg_data = cfg_data.replace(
                '#MIDI.LOWER.CHANNEL#', str(1 + midi_chans[1]))
            cfg_data = cfg_data.replace(
                '#MIDI.PEDALS.CHANNEL#', str(1 + midi_chans[2]))
            cfg_data = cfg_data.replace(
        ...
        if not self.proc:
            ...
            if self.manuals_config[4][0]:
                ...
                if len(self.layers) == i:
                    try:
                        ch = midi_chans[1] = self.zyngui.screens['layer'].get_next_free_midi_chan(
                            ch)
                        logging.info(
                            "Lower Manual Layer in chan {}".format(midi_chans[1]))
                        self.zyngui.screens['layer'].add_layer_midich(
                            midi_chans[1], False)
                        ...
                        logging.error(
                            "Lower Manual Layer can't be added! => {}".format(e))
                else:
            ...
            if self.manuals_config[4][1]:
                ...
                if len(self.layers) == i:
                    try:
                        # Adding Pedal Layer
                        midi_chans[2] = self.zyngui.screens['layer'].get_next_free_midi_chan(
                            ch)
                        logging.info(
                            "Pedal Layer in chan {}".format(midi_chans[2]))
                        self.zyngui.screens['layer'].add_layer_midich(
                            midi_chans[2], False)
                        ...
                        logging.error(
                            "Pedal Layer can't be added! => {}".format(e))
                else:
            ...
            if midi_prog and isinstance(midi_prog, int):
                logging.debug(
                    "Loading manuals configuration program: {}".format(midi_prog))
    ...
    def update_controller_values(self, layer, preset):
        ...
        for param, v in preset[3].items():
            ...
            try:
                ...
                logging.debug(
                    "Can't update controller '{}' => {}".format(zcsymbol, e))
