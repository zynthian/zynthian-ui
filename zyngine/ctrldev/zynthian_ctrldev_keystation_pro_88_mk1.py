...
class zynthian_ctrldev_keystation_pro_88_mk1(zynthian_ctrldev_base):
   ...
   def midi_event(self, ev):
         ...
         # logger.debug(f"MIDI Event empfangen: {ev}")
         if len(ev) > 0:
         ...
         if len(ev) == 3: # most times 3 bytes and we need 3 bytes
               ...
               if status == 0xb0 and data1 == 42:  # 42 is knob 18 in "keystations Preset 10. (Press Recall and choose 10)"
               ...
               if status == 0xb0 and data1 == 34:   # 34 is knob 19 in "Preset-Recall 10"
                  pot = data2-self.zynpot_1
                  self.zynpot_1 = data2
                  self.state_manager.send_cuia("ZYNPOT", [1, pot])
                  return True # Event processed. restarts event loop
               
               if ev[0] == 0xb0 and data1 == 10:   # 10 is knob 10 in "Preset-Recall 10"
                  pot = data2-self.zynpot_2
                  self.zynpot_2 = data2
                  self.state_manager.send_cuia("ZYNPOT", [2, pot])
                  return True # Event processed. restarts event loop
               
               if ev[0] == 0xb0 and data1 == 2:    # 2 is knob 11 in "Preset-Recall 10"
