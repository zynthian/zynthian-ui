...
class zynthian_ctrldev_korg_nanokontrol2(zynthian_ctrldev_zynmixer):
	...
	def refresh(self):
		...
		for i in range(0, 8):
			...
			lib_zyncore.dev_send_ccontrol_change(self.idev_out, self.midi_chan, self.mute_ccnums[i], mute)
			lib_zyncore.dev_send_ccontrol_change(self.idev_out, self.midi_chan, self.solo_ccnums[i], solo)
			lib_zyncore.dev_send_ccontrol_change(self.idev_out, self.midi_chan, self.rec_ccnums[i], rec)
		...
		if evtype == 0xB:
					...
					if not self.rec_mode:
						...
						if mixer_chan is not None:
							...
							if self.idev_out is not None:
								...
								lib_zyncore.dev_send_ccontrol_change(self.idev_out, self.midi_chan, ccnum, val)
						elif self.idev_out is not None:
							# If not associated mixer channel, turn-off the led
							lib_zyncore.dev_send_ccontrol_change(self.idev_out, self.midi_chan, ccnum, 0)
				return True
			...
			#	return True
				...
				if self.shift:
					...
					if col == 7:
						self.zynmixer.set_balance(255, 2.0 * ccval / 127.0 - 1.0)
