...
class zynthian_engine_linuxsampler(zynthian_engine):
    ...
    _ctrl_screens = [
        ...
        ['portamento', ['legato on/off', 'portamento on/off',
    ...
    def ls_init(self):
        try:
            ...
            self.ls_audio_device_id = self.lscp_send_single(
                "CREATE AUDIO_OUTPUT_DEVICE JACK ACTIVE='true' CHANNELS='16' NAME='{}'".format(self.jackname))
            for i in range(8):
                self.lscp_send_single("SET AUDIO_OUTPUT_CHANNEL_PARAMETER {} {} NAME='CH{}_1'".format(
                    self.ls_audio_device_id, i*2, i))
                self.lscp_send_single("SET AUDIO_OUTPUT_CHANNEL_PARAMETER {} {} NAME='CH{}_2'".format(
        ...
        if layer.ls_chan_info:
            ...
            if ls_engine != layer.ls_chan_info['ls_engine']:
                try:
                    ...
                    self.lscp_send_single(
                        "SET CHANNEL AUDIO_OUTPUT_CHANNEL {} 1 {}".format(ls_chan_id, i*2+1))
        ...
        if os.path.isdir(dpath):
            ...
            try:
                sfz_files = check_output(
                    "find \"{}\" -type f -iname *.sfz".format(dpath), shell=True).decode("utf-8").split("\n")
