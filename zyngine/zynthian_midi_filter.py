...
class MidiFilterRule:
    ...
    def parse_rule(self, rule, set_rules=True):
        ...
        parts = rule.split("\n")
        if len(parts) > 1:
            raise MidiFilterException(
                "Invalid rule format. Multi-line rules are not allowed.")
        ...
        if self.rule_type in ("IGNORE", "CLEAN"):
            if len(parts) > 3:
                raise MidiFilterException(
                    "Invalid rule format. Too many parts.")
            # Parse arguments
        ...
        # MAP rule ...
            ...
            try:
                ...
                self.args.append(MidiFilterArgs(
                    parts[arrow_i+1:], self.args[0]))
                ...
                if n0 > 1 and n1 == 1:
                    ...
                    raise Exception(
                        "MAP rule channel lists can't be matched ({}=>{})".format(n0, n1))
                ...
                if self.args[0].ev_type is None and self.args[1].ev_type is None:
                    ...
                    if m1 == 1:
                        ...
                        raise Exception(
                            "MAP rule event lists can't be matched ({}=>{}".format(m0, m1))
                    logging.debug("Mapping {} events to {} ...".format(m0, m1))

            except Exception as e:
                raise MidiFilterException(
                    "Invalid MAP rule format (%s): %s" % (rule, e))
        else:
            raise MidiFilterException(
                "Invalid RULE type (%s)" % self.rule_type)
    ...
    def set_rules(self, set_rules=True):
        ...
        if self.rule_type in ("IGNORE", "CLEAN"):
            if self.args[0].ev_type:
                ev_types = [
                    MidiFilterArgs.EVENT_TYPE_CODES[self.args[0].ev_type]]
            else:
            ...
            for ch in self.args[0].ch_list:
                for ev_type in ev_types:
                    ...
                    for ev_num in ev_list:
                        n_rules += 1
                        logging.debug("%s CH#%s %s#%s" %
                                      (self.rule_type, ch, ev_type, ev_num))
                        if set_rules:
                            if self.rule_type == "IGNORE":
                                zyncoder.lib_zyncoder.set_midi_filter_event_ignore(
                                    ev_type, ch, ev_num)
                            elif self.rule_type == "CLEAN":
                                zyncoder.lib_zyncoder.del_midi_filter_event_map(
                                    ev_type, ch, ev_num)
            ...
            if self.args[0].ev_type and self.args[1].ev_type:
                ev1_types = [
                    MidiFilterArgs.EVENT_TYPE_CODES[self.args[0].ev_type]]
                ev2_types = [
                    MidiFilterArgs.EVENT_TYPE_CODES[self.args[1].ev_type]]
            else:
            ...
            for ch1, ch2 in zip(self.args[0].ch_list, self.args[1].ch_list):
                for ev1_type, ev2_type in zip(ev1_types, ev2_types):
                    ...
                    for ev1_num, ev2_num in zip(ev1_list, ev2_list):
                        n_rules += 1
                        logging.debug("MAP CH#%s %s#%s => CH#%s %s#%s" % (
                            ch1, ev1_type, ev1_num, ch2, ev2_type, ev2_num))
                        if set_rules:
                            zyncoder.lib_zyncoder.set_midi_filter_event_map(
                                ev1_type, ch1, ev1_num, ev2_type, ch2, ev2_num)
...
class MidiFilterScript:
    ...
    def parse_script(self, script, set_rules=True):
        ...
        if isinstance(script, str):
            ...
            raise MidiFilterException(
                "Script must be a String or a List of Strings")
        for rule in script:
            rule = rule.strip()
            if len(rule) > 0:
                ...
                if len(rule) > 8:
                    ...
                    raise MidiFilterException(
                        "Script Rule is too short to be valid")
...
class TestMidiFilterRule(unittest.TestCase):
    ...
    def test_map_rules(self):
        ...
        self.assertTrue(mfr.set_rules() == 18)
        mfr = MidiFilterRule(
            "MAP CH#1,2,3:8 CC#1:3,7,8 => CH#5:10,11,12 CC#1,2,4:6", False)
        self.assertTrue(mfr.set_rules() == 40)
