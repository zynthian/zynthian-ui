...
class zynthian_engine(zynthian_basic_engine):
    ...
    def osc_init(self, target_port=None, proto=liblo.UDP):
        ...
        try:
            self.osc_target = liblo.Address(
                'localhost', self.osc_target_port, proto)
            ...
            logging.info("OSC server running in port %s" %
                         str(self.osc_server_port))
            self.osc_add_methods()
            self.osc_server.start()
        except liblo.AddressError as err:
            logging.error(
                "OSC Server can't be initialized (%s). Running without OSC feedback." % err)
    ...
    def get_preset_list(self, bank):
        logging.info('Getting Preset List for %s: NOT IMPLEMENTED!' %
                     self.name), 'PD'
            ...
            try:
                ...
                if len(ctrl_set) >= 4:
                    ...
                    self._ctrl_screens.append(
                        ["{}#{}".format(self.nickname, c), ctrl_set])
                    ctrl_set = []
                    c = c+1
            except Exception as err:
                logging.error(
    ...
    def init_midi_learn(self, zctrl):
        logging.info("Learning '{}' ({}) ...".format(
            zctrl.symbol, zctrl.get_path()))
