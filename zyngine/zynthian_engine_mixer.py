...
class zynthian_engine_mixer(zynthian_engine):
    ...
    def get_controllers_dict(self, layer, ctrl_list=None):
        ...
        if self.allow_headphones() and self.zyngui and self.zyngui.get_zynthian_config("rbpi_headphones"):
            try:
                zctrls_headphones = self.get_mixer_zctrls(
                ...
                logging.error(
                    "Can't configure headphones volume control: {}".format(e))
    ...
    def get_mixer_zctrls(self, device_name, ctrl_list):
        ...
        try:
            ctrls = check_output("amixer -M -c {}".format(device_name),
                                 shell=True).decode("utf-8").split("Simple mixer control ")
            for ctrl in ctrls:
        ...
        try:
            if zctrl.labels:
                if zctrl.graph_path[1] == "VToggle":
                    amixer_command = "amixer -M -c {} set '{}' '{}%'".format(
                        self.device_name, zctrl.graph_path[0], zctrl.value)
                else:
                    amixer_command = "amixer -M -c {} set '{}' '{}'".format(
                        self.device_name, zctrl.graph_path[0], zctrl.get_value2label())
            else:
                    ...
                    for i in range(0, nchans):
                        ...
                        if symbol_i in self.zctrls:
                            values.append("{}%".format(
                                self.zctrls[symbol_i].value))
                        else:
                            values.append("0%")
                else:
                    values.append("{}%".format(zctrl.value))

                amixer_command = "amixer -M -c {} set '{}' '{}' {} unmute".format(
                    devname, zctrl.graph_path[0], zctrl.graph_path[1], ','.join(values))
