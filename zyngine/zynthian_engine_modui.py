...
class zynthian_engine_modui(zynthian_engine):
    ...
    def stop(self):
        ...
        if self.is_service_active("mod-ui"):
            ...
            check_output(
                ("systemctl stop mod-host && systemctl stop browsepy && systemctl stop mod-ui"), shell=True)
                    ...
                    try:
                        ...
                        if len(ctrl_set) >= 4:
                            ...
                            self._ctrl_screens.append(
                                [self.plugin_info[pgraph]['name']+'#'+str(c), ctrl_set])
                            ctrl_set = []
                            c = c+1
                    except Exception as err:
                        logging.error(
                            "Generating Controller Screens: %s => %s" % (pgraph, err))
                if len(ctrl_set) >= 1:
                    # logging.debug("ADDING CONTROLLER SCREEN #"+str(c))
                    self._ctrl_screens.append(
                        [self.plugin_info[pgraph]['name']+'#'+str(c), ctrl_set])
    ...
    def send_controller_value(self, zctrl):
        self.websocket.send("param_set %s %.6f" %
                            (zctrl.graph_path, zctrl.value))
        logging.debug("WS << param_set %s %.6f" %
                      (zctrl.graph_path, zctrl.value))
                ...
                if self.is_service_active("mod-ui"):
                    try:
                        ...
                        if error_counter > 100:
                            ...
                            self.start()
                            self.set_bank(
                                self.layers[0], self.layers[0].bank_info)
                            error_counter = 0
                        else:
                            error_counter += 1
                            sleep(1)
    ...
    def api_get_request(self, path, data=None, json=None):
        ...
        if res.status_code != 200:
            logging.error("GET call to MOD-UI API: " +
    ...
    def add_plugin_cb(self, pgraph, puri, posx, posy):
        ...
        if pinfo:
            ...
            pinfo['ports']['control']['input'].insert(
                0, {'symbol': ':bypass', 'ctrl': bypass_zctrl})
            # Add position info
    ...
    def init_midi_learn(self, zctrl):
        ...
        res = self.api_post_request("/effect/parameter/address/"+zctrl.graph_path,
