...
class zynthian_layer:
    ...
    def send_ctrl_midi_cc(self):
        for k, zctrl in self.controllers_dict.items():
            if zctrl.midi_cc:
                self.zyngui.zynmidi.set_midi_control(
                    zctrl.midi_chan, zctrl.midi_cc, int(zctrl.value))
                logging.debug("Sending MIDI CC{}={} for {}".format(
                    zctrl.midi_cc, zctrl.value, k))
    ...
    def restore_snapshot_1(self, snapshot):
        ...
        try:
            # tweak for working with setbfree extended config!! => TODO improve it!!
            self.bank_name = snapshot['bank_name']
            self.load_bank_list()
            self.bank_name = None
            self.set_bank_by_name(snapshot['bank_name'])

        except Exception as e:
            logging.warning("Invalid Bank on layer {}: {}".format(
                self.get_basepath(), e))
        ...
        try:
            ...
            self.preset_loaded = self.set_preset_by_name(
                snapshot['preset_name'])
        except Exception as e:
            logging.warning("Invalid Preset on layer {}: {}".format(
                self.get_basepath(), e))
    ...
    def restore_snapshot_2(self, snapshot):
        ...
        for k in snapshot['controllers_dict']:
            try:
                self.controllers_dict[k].restore_snapshot(
                    snapshot['controllers_dict'][k])
            except Exception as e:
                logging.warning("Invalid Controller on layer {}: {}".format(
                    self.get_basepath(), e))
    ...
    def restore_zs3(self, i):
        ...
        if zs3:
            ...
            for k in zs3['controllers_dict']:
                self.controllers_dict[k].restore_snapshot(
                    zs3['controllers_dict'][k])

            # Set Note Range
            if self.midi_chan >= 0 and 'note_range' in zs3:
                nr = zs3['note_range']
                zyncoder.lib_zyncoder.set_midi_filter_note_range(
                    self.midi_chan, nr['note_low'], nr['note_high'], nr['octave_trans'], nr['halftone_trans'])
    ...
    def add_audio_out(self, jackname):
        ...
        if jackname not in self.audio_out:
            ...
            logging.debug("Connecting Audio Output {} => {}".format(
                self.get_audio_jackname(), jackname))
    ...
    def del_audio_out(self, jackname):
        ...
        try:
            ...
            logging.debug("Disconnecting Audio Output {} => {}".format(
                self.get_audio_jackname(), jackname))
        except:
            pass
    ...
    def add_audio_in(self, jackname):
        if jackname not in self.audio_in:
            ...
            logging.debug("Connecting Audio Capture {} => {}".format(
                jackname, self.get_audio_jackname()))
    ...
    def del_audio_in(self, jackname):
        try:
            ...
            logging.debug("Disconnecting Audio Capture {} => {}".format(
                jackname, self.get_audio_jackname()))
        except:
            pass
    ...
    def add_midi_out(self, jackname):
        ...
        if jackname not in self.midi_out:
            ...
            logging.debug("Connecting MIDI {} => {}".format(
                self.get_midi_jackname(), jackname))
    ...
    def del_midi_out(self, jackname):
        ...
        try:
            ...
            logging.debug("Disconnecting MIDI {} => {}".format(
                self.get_midi_jackname(), jackname))
        except:
            pass
