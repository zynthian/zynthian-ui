...
def get_pianoteq_binary_info():
    ...
    if check_pianoteq_binary():
        ...
        proc = subprocess.Popen(
            [PIANOTEQ_BINARY, "--version"], stdout=subprocess.PIPE)
        ...
        try:
            ...
            if audio_setup_node:
                ...
                for devicesetup in audio_setup_node.iter('DEVICESETUP'):
                    ...
                    devicesetup.set('audioDeviceRate',
                                    str(PIANOTEQ_SAMPLERATE))
...
PIANOTEQ_SW_DIR = os.environ.get(
    'ZYNTHIAN_SW_DIR', "/zynthian/zynthian-sw") + "/pianoteq6"
...
try:
    PIANOTEQ_VERSION = list(
        map(int, os.environ.get('PIANOTEQ_VERSION').split(".")))
...
if PIANOTEQ_PRODUCT == "STANDARD":
    ...
    PIANOTEQ_CONFIG_FILENAME = "{} {}.prefs".format(
        PIANOTEQ_NAME, PIANOTEQ_PRODUCT)
...
class zynthian_engine_pianoteq(zynthian_engine):
    ...
    bank_list = [
        ...
        ('H. Ruckers II Harpsichord', 27,
         'H. Ruckers II Harpsichord', 'Harpsichord:A'),
    ...
    _ctrls = [
        ...
        ['sostenuto', 66, 'off', [
            ['off', '1/4', '1/2', '3/4', 'full'], [0, 25, 51, 76, 102]]],
    ...
    def __init__(self, zyngui=None, update_presets_cache=False):
        ...
        if not os.path.isfile(PIANOTEQ_CONFIG_FILE):
            logging.debug(
                "Pianoteq configuration does not exist. Creating one...")
            ensure_dir(PIANOTEQ_CONFIG_DIR + "/")
            if os.path.isfile(self.data_dir + "/pianoteq6/" + PIANOTEQ_CONFIG_FILENAME):
                shutil.copy(self.data_dir + "/pianoteq6/" +
                            PIANOTEQ_CONFIG_FILENAME, PIANOTEQ_CONFIG_DIR)
            else:
                shutil.copy(self.data_dir +
                            "/pianoteq6/Pianoteq6.prefs", PIANOTEQ_CONFIG_FILE)
    ...
    def prepare_banks(self):
        ...
        if not PIANOTEQ_TRIAL:
            ...
            if subl:
                ...
                self.bank_list = licensed_banks + free_banks + \
                    self.spacer_demo_bank + unlicensed_banks
    ...
    def save_presets_cache(self):
        ...
        try:
            pianoteq = subprocess.Popen(
                [PIANOTEQ_BINARY, "--list-presets"], stdout=subprocess.PIPE)
            bank_list = sorted(self.bank_list, key=lambda bank: len(
                bank[0]) if bank[0] else 0, reverse=True)
        ...
        try:
            ...
            logging.info("Saving presets cache '%s' => \n%s" %
                         (self.presets_cache_fpath, json))
        except Exception as e:
            logging.error(
                "Can't generate JSON while saving presets cache: %s" % e)
            return False
        # Write to file
        ...
        try:
            ...
            logging.error("Can't save presets cache '%s': %s" % (
                self.presets_cache_fpath, e))
            return False
        return True
    ...
    def load_presets_cache(self):
        ...
        try:
            ...
            logging.error("Can't load presets cache '%s': %s" % (
                self.presets_cache_fpath, e))
            return False
        # Decode JSON
        try:
            ...
            logging.error(
                "Can't decode JSON while loading presets cache: %s" % e)
            return False
        return True
            ...
            for f in cls.user_presets_flist:
                if (isfile(join(cls.user_presets_dpath, f)) and f[-4:].lower() == ".fxp"):
                    ...
                    if bank_prefix == fname[0:len(bank_prefix)]:
                        ...
                        preset_title = dbank + "/" + \
                            str.replace(
                                fname[len(bank_prefix):-4], '_', ' ').strip()
                        user_presets.append(
                            [preset_path, None, preset_title, None, dbank])
        return user_presets
    ...
    def get_preset_list(self, bank):
        ...
        if bank_name in self.presets:
            logging.info("Getting Preset List for %s [%s]" % (
                self.name, bank_name))
            res = copy.deepcopy(self.presets[bank_name])
        else:
            logging.error("Can't get Preset List for %s [%s]" % (
                self.name, bank_name))
            res = []
        return res
    ...
    def generate_presets_midimapping(self):
        ...
        if not os.path.isfile(PIANOTEQ_MIDIMAPPINGS_DIR + "/ZynthianControllers.ptm"):
            logging.debug(
                "Pianoteq Base MIDI-Mapping does not exist. Creating ...")
            ...
            if bank[0] in self.presets:
                for prs in self.presets[bank[0]]:
                    try:
                        ...
                        midi_event_str = bytes(
                            "Program Change " + str(len(data)+1), "utf8")
                        parts = prs[0].split('/')
                        if len(parts) > 1:
                            action_str = bytes("{{LoadPreset|28|{}|{}|0}}".format(
                                parts[0], parts[1]), "utf8")
                        else:
                            action_str = bytes(
                                "{{LoadPreset|28||{}|0}}".format(prs[0]), "utf8")
                        row = b'\x01\x00\x00\x00'
                        row += struct.pack("<I",
                                           len(midi_event_str)) + midi_event_str
    ...
    def create_midimapping_file(self, mmn, data):
        ...
        shutil.copy(PIANOTEQ_MIDIMAPPINGS_DIR +
                    "/ZynthianControllers.ptm", fpath)
    ...
    def zynapi_rename_preset(cls, preset_path, new_preset_name):
        ...
        for b in cls.get_user_banks():
            if fname.startswith(b[2]):
                new_preset_path = head + "/" + \
                    b[2] + " " + new_preset_name + ext
