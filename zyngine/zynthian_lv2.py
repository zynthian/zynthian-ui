...
JALV_LV2_CONFIG_FILE_ALL = "{}/jalv/all_plugins.json".format(
    os.environ.get('ZYNTHIAN_CONFIG_DIR'))
...
def is_plugin_ui(plugin):
    for uri in plugin.get_data_uris():
        try:
            ...
            logging.error("Failed to get UI for plugin %s",
                          str(plugin.get_name()))
            pass
    return False
...
def _generate_plugin_presets_cache(plugin):
    ...
    # Save cache file
    fpath_cache = "{}/jalv/presets_{}.json".format(
        os.environ.get('ZYNTHIAN_CONFIG_DIR'), sanitize_fname(plugin_name))
    try:
