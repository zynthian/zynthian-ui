...
class zynthian_controller:
        ...
        if self.labels:
            ...
            if not self.ticks:
                ...
                if self.is_integer:
                    for i in range(n):
                        self.ticks.append(
                            self.value_min+int(i*(self.value_max+1)/n))
                else:
                    for i in range(n):
                ...
                if force_sending:
                    try:
                        ...
                        if self.osc_path:
                            ...
                            logging.debug(
                                "Sending OSC controller '{}' value => {}".format(self.symbol, val))

                        elif self.midi_cc:
                            zyncoder.lib_zyncoder.ui_send_ccontrol_change(
                                self.midi_chan, self.midi_cc, mval)
                            logging.debug("Sending MIDI controller '{}' value => {} ({})".format(
                                self.symbol, val, mval))

                    except Exception as e:
                        logging.warning(
                            "Can't send controller '{}' value: {} => {}".format(self.symbol, val, e))

            # Send feedback to MIDI controllers
            try:
                if self.midi_learn_cc:
                    zyncoder.lib_zyncoder.ctrlfb_send_ccontrol_change(
                        self.midi_learn_chan, self.midi_learn_cc, mval)
    ...
    def get_value2index(self, val=None):
        ...
        try:
            if self.ticks:
                ...
                i = min(int((val-self.value_min)*len(self.labels) /
                        self.value_range), len(self.labels)-1)
    ...
    def get_ctrl_midi_val(self):
        try:
            if self.is_logarithmic:
                val = int(127*math.log(self.value/self.value_min) /
                          self.log_powbase)
            else:
                val = min(
    ...
    def restore_snapshot(self, snapshot):
        if isinstance(snapshot, dict):
            ...
            if 'midi_learn_chan' in snapshot and 'midi_learn_cc' in snapshot:
                ...
                # Restore MIDI-learn
                self.set_midi_learn(int(snapshot['midi_learn_chan']), int(
                    snapshot['midi_learn_cc']))
        else:
    ...
    def _set_midi_learn(self, chan, cc):
        logging.info(
            "MIDI-CC SET '{}' => {}, {}".format(self.symbol, chan, cc))
    ...
    def _unset_midi_learn(self):
        logging.info("MIDI-CC UNSET '{}' => {}, {}".format(self.symbol,
                     self.midi_learn_chan, self.midi_learn_cc))
    ...
    def midi_learn_zyncoder(self, chan, cc):
        try:
            if zyncoder.lib_zyncoder.set_midi_filter_cc_swap(ctypes.c_ubyte(chan), ctypes.c_ubyte(cc), ctypes.c_ubyte(self.midi_chan), ctypes.c_ubyte(self.midi_cc)):
                logging.info("Set MIDI filter CC map: (%s, %s) => (%s, %s)" % (
                    chan, cc, self.midi_chan, self.midi_cc))
                return self._set_midi_learn(chan, cc)
            else:
                logging.error(
                    "Can't set MIDI filter CC swap map: call returned 0")

        except Exception as e:
            logging.error("Can't set MIDI filter CC swap map: (%s, %s) => (%s, %s) => %s" % (
                self.midi_learn_chan, self.midi_learn_cc, self.midi_chan, self.midi_cc, e))
    ...
    def midi_unlearn_zyncoder(self):
        try:
            if zyncoder.lib_zyncoder.del_midi_filter_cc_swap(ctypes.c_ubyte(self.midi_learn_chan), ctypes.c_ubyte(self.midi_learn_cc)):
                logging.info("Deleted MIDI filter CC map: {}, {}".format(
                    self.midi_learn_chan, self.midi_learn_cc))
                return self._unset_midi_learn()
            else:
                logging.error(
                    "Can't delete MIDI filter CC swap map: Call returned 0")

        except Exception as e:
            logging.error("Can't delete MIDI filter CC swap map: {}, {} => {}".format(
                self.midi_learn_chan, self.midi_learn_cc, e))
    ...
    def refresh_gui(self):
        ...
        try:
            if (self.engine.zyngui.active_screen == 'control' and not self.engine.zyngui.modal_screen) or self.engine.zyngui.modal_screen == 'alsa_mixer':
                self.engine.zyngui.screens['control'].set_controller_value(
                    self)
        except Exception as e:
            logging.debug(e)
