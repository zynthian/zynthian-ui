...
class zynthian_gui_controller:
    ...
    def unset_hl(self):
        try:
            self.canvas.itemconfig(
                self.arc, outline=zynthian_gui_config.color_ctrl_bg_on)
        except:
            pass
    ...
    def calculate_plot_values(self):
        ...
        if self.zctrl.labels:
                    ...
                    if self.selmode:
                        ...
                        for i in reversed(range(self.n_values)):
                            if val <= self.zctrl.ticks[i]:
                                break
                        valplot = self.scale_plot * \
                            (self.max_value-self.zctrl.ticks[i])
                        val = self.zctrl.ticks[i]
                    else:
                        for i in range(self.n_values-1):
                            if val < self.zctrl.ticks[i+1]:
                                valplot = self.scale_plot * \
                                    (self.zctrl.ticks[i]-self.zctrl.ticks[0])
                                break
                        if valplot == None:
                            i += 1
                            valplot = self.scale_plot * \
            ...
            if self.zctrl.midi_cc == 0:
                ...
                if self.logarithmic:
                    val = self.zctrl.value_min * \
                        pow(self.scale_value, self.value/self.n_values)
                else:
    ...
    def plot_value_rectangle(self):
        ...
        if self.rectangle:
            ...
            self.rectangle_bg = self.canvas.create_rectangle(
                (x1, y1, x1+lx, y2), fill=zynthian_gui_config.color_ctrl_bg_off, width=0)
            self.rectangle = self.canvas.create_rectangle(
                (x1, y1, x2, y2), fill=zynthian_gui_config.color_ctrl_bg_on, width=0)
        ...
        if self.value_text:
            ...
            self.value_text = self.canvas.create_text(x1+self.trw/2-1, y1-self.trh, width=self.trw,
                                                      ...
                                                      font=(
                                                          zynthian_gui_config.font_family, self.value_font_size),
    ...
    def plot_value_triangle(self):
        ...
        if self.triangle:
            ...
            self.triangle_bg = self.canvas.create_polygon(
                (x1, y1, x1+self.trw, y1, x1+self.trw, y1-self.trh), fill=zynthian_gui_config.color_ctrl_bg_off)
            self.triangle = self.canvas.create_polygon(
                (x1, y1, x2, y1, x2, y2), fill=zynthian_gui_config.color_ctrl_bg_on)
        ...
        if not self.midi_bind:
            self.midi_bind = self.canvas.create_text(
                ...
                font=(zynthian_gui_config.font_family, int(
                    0.7*zynthian_gui_config.font_size)),
                text=str(midi_cc))
        else:
            self.canvas.itemconfig(
                self.midi_bind, text=str(midi_cc), fill=color)
    ...
    def set_midi_bind(self):
        if self.zctrl.midi_cc == 0:
            ...
            swap_info = zyncoder.lib_zyncoder.get_midi_filter_cc_swap(
                self.zctrl.midi_chan, self.zctrl.midi_cc)
    ...
    def set_title(self, tit):
        ...
        rfont = tkFont.Font(
            family=zynthian_gui_config.font_family, size=max_fs)
        if n_words == 1:
            maxlen = rfont.measure(self.title)
        elif n_words == 2:
            maxlen = max([rfont.measure(w) for w in words])
        elif n_words == 3:
            maxlen = max([rfont.measure(w)
                         for w in [words[0]+' '+words[1], words[1]+' '+words[2]]])
            max_fs = max_fs-1
        elif n_words >= 4:
            maxlen = max([rfont.measure(w)
                         for w in [words[0]+' '+words[1], words[2]+' '+words[3]]])
            max_fs = max_fs-1
    ...
    def calculate_value_font_size(self):
        if self.zctrl.labels:
            ...
            if maxlen > 3:
                rfont = tkFont.Font(
                    family=zynthian_gui_config.font_family, size=zynthian_gui_config.font_size)
            ...
            if self.format_print:
                ...
                maxlen = max(len(str(self.zctrl.value_min)),
                             len(str(self.zctrl.value_max)))
            if maxlen > 5:
                font_scale = 0.8
            elif maxlen > 4:
                font_scale = 0.9
        ...
        if self.value_text:
            self.canvas.itemconfig(self.value_text, font=(
                zynthian_gui_config.font_family, self.value_font_size))
    ...
    def config(self, zctrl):
        ...
        logging.debug("ZCTRL '%s': %s (%s -> %s), %s, %s" % (zctrl.short_name,
                      zctrl.value, zctrl.value_min, zctrl.value_max, zctrl.labels, zctrl.ticks))
            ...
            if zctrl.midi_cc == 0:
                ...
                # Float
                    ...
                    if self.logarithmic:
                        ...
                        val = self.n_values * \
                            math.log(zctrl.value/zctrl.value_min) / \
    ...
    def zctrl_sync(self, set_zyncoder=True):
        ...
        # Numeric value
            ...
            if self.zctrl.midi_cc == 0:
                ...
                val = self.n_values * \
                    math.log(self.zctrl.value/self.zctrl.value_min) / \
                    self.log_scale_value
            else:
    ...
    def setup_zyncoder(self):
        ...
        try:
            if isinstance(self.zctrl.osc_path, str):
                ...
                midi_cc = None
                zyn_osc_path = "{}:{}".format(
                    self.zctrl.osc_port, self.zctrl.osc_path)
            ...
            if zyncoder.lib_zyncoder:
                ...
                zyncoder.lib_zyncoder.setup_zyncoder(self.index, pin_a, pin_b, self.zctrl.midi_chan, midi_cc, osc_path_char, int(
                    self.mult*self.value), int(self.mult*(self.max_value-self.val0)), self.step)
        except Exception as err:
            logging.error("%s" % err)
        ...
        if self.value is None or self.value != v:
            ...
            if self.shown:
                if set_zyncoder and zyncoder.lib_zyncoder:
                    if self.mult > 1:
                        v = self.mult*v
                    zyncoder.lib_zyncoder.set_value_zyncoder(
                        self.index, ctypes.c_uint(int(v)), int(send_zyncoder))
    ...
    def cb_canvas_release(self, event):
        if self.canvas_push_ts:
            ...
            logging.debug("CONTROL %d RELEASE => %s, %s" %
                          (self.index, dts, motion_rate))
            if motion_rate < 10:
                if dts < 0.3:
