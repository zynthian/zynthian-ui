...
class zynthian_gui_admin(zynthian_gui_selector):
    ...
    def fill_list(self):
        ...
        if zynthian_gui_config.snapshot_mixer_settings:
            self.list_data.append(
                (self.toggle_snapshot_mixer_settings, 0, "[x] Audio Levels on Snapshots"))
        else:
            self.list_data.append(
        ...
        if zynthian_gui_config.midi_filter_output:
            self.list_data.append(
                (self.toggle_midi_filter_output, 0, "[x] Route MIDI to Output"))
        else:
            self.list_data.append(
                (self.toggle_midi_filter_output, 0, "[  ] Route MIDI to Output"))
    ...
    def toggle_midi_sys(self):
        ...
        zyncoder.lib_zyncoder.set_midi_filter_system_events(
...
# ------------------------------------------------------------------------------
    ...
    def is_update_available(self):
        repos = ["/zynthian/zyncoder", "/zynthian/zynthian-ui", "/zynthian/zynthian-sys",
                 "/zynthian/zynthian-webconf", "/zynthian/zynthian-data"]
        update_available = False
        for path in repos:
            update_available |= (check_output(
                "git -C %s status --porcelain -bs | grep behind | wc -l" % path, shell=True).decode()[:1] == '1')
