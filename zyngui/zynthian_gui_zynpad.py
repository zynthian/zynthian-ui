...
class zynthian_gui_zynpad(zynthian_gui_base.zynthian_gui_base):
	...
	def refresh_pad(self, pad, force=False):
		...
		changed_state = self.zynseq.libseq.hasSequenceChanged(self.bank, pad)
	...
	def get_xy_from_pad(self, pad):
		col = pad // self.zynseq.col_in_bank
		row = pad % self.zynseq.col_in_bank
		return (col, row)


	def get_pad_from_xy(self, col, row):
		if col < self.zynseq.col_in_bank and row < self.zynseq.col_in_bank:
			return col * self.zynseq.col_in_bank + row
		else:
			return -1
	...
	def set_trigger_device(self, idev=None):
		if idev is None:
			idev = self.zynseq.libseq.getTriggerDevice()
		if idev != self.trigger_device:
			...
			if self.trigger_device > 0:
				...
				get_lib_zyncore().zmip_set_route_extdev(self.trigger_device-1, 1)
			...
			self.zynseq.libseq.setTriggerDevice(idev)

			# New selected device => Unroute & add feedback list
			if self.trigger_device > 0:
				get_lib_zyncore().zmip_set_route_extdev(self.trigger_device-1, 0)
	...
	def update_launchpad_mini_bank(self):
		...
		for row in range(0,8):
			note = 16 * row + col
			if row == self.bank - 1:
				get_lib_zyncore().ctrlfb_send_note_on(0, note, 0x3C)
			else:
				get_lib_zyncore().ctrlfb_send_note_on(0, note, 0xC)
	...
	def update_launchpad_mini_pad(self, pad, state, mode):
		...
		if mode == 0:
			...
			vel = 0xC
		get_lib_zyncore().ctrlfb_send_note_on(0, note, vel)
	...
	def midi_event_launchpad_mini(self, ev):
		...
		if evtype == 9:
			...
			if pad >= 0:
				self.zynseq.libseq.togglePlayState(self.bank, pad)
				return True
	...
	def init_launchkey_mini_3(self):
		...
		get_lib_zyncore().ctrlfb_send_note_on(15, 12, 127)
	...
	def end_launchkey_mini_3(self):
		...
		get_lib_zyncore().ctrlfb_send_note_on(15, 12, 0)
	...
	def update_launchkey_mini_3_bank(self):
		...
		for row in range(2):
			...
			for col in range(8):
				...
				if too_big:
					note = 96 + row * 16 + col
					get_lib_zyncore().ctrlfb_send_note_on(0, note, 0)
				else:
	...
	def update_launchkey_mini_3_pad(self, pad, state, mode):
		...
		try:
			...
			if mode == 0 or group > 16:
				...
				get_lib_zyncore().ctrlfb_send_note_on(2, note, PAD_COLOURS[group])
			elif state == zynseq.SEQ_STARTING:
				get_lib_zyncore().ctrlfb_send_note_on(1, note, STARTING_COLOUR)
			elif state == zynseq.SEQ_STOPPING:
				get_lib_zyncore().ctrlfb_send_note_on(1, note, STOPPING_COLOUR)
			elif state == zynseq.SEQ_STOPPED:
				get_lib_zyncore().ctrlfb_send_note_on(0, note, PAD_COLOURS[group])
	...
	def show_menu(self):
		...
		options['Grid size ({}x{})'.format(self.zynseq.col_in_bank, self.zynseq.col_in_bank)] = 'Grid size'
