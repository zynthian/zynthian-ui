...
class zynthian_gui_zynpad():
    ...
    def __init__(self, parent):
        ...
        self.selection = self.grid_canvas.create_rectangle(
            0, 0, self.column_width, self.row_height, fill="", outline=SELECT_BORDER, width=self.select_thickness, tags="selection")
    ...
    def populate_menu(self):
        self.parent.add_menu({'Play mode': {'method': self.parent.show_param_editor, 'params': {'min': 0, 'max': len(
            zynthian_gui_stepsequencer.PLAY_MODES)-1, 'get_value': self.get_selected_pad_mode, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'MIDI channel': {'method': self.parent.show_param_editor, 'params': {
                             'min': 1, 'max': 16, 'get_value': self.get_pad_channel, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'Trigger channel': {'method': self.parent.show_param_editor, 'params': {
                             'min': 0, 'max': 16, 'get_value': self.get_trigger_channel, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'Trigger note': {'method': self.parent.show_param_editor, 'params': {
                             'min': -1, 'max': 128, 'get_value': self.get_trigger_note, 'on_change': self.on_menu_change, 'on_reset': self.reset_trigger_note}}})
        self.parent.add_menu({'-------------------': {}})
        self.parent.add_menu({'Grid size': {'method': self.parent.show_param_editor, 'params': {
                             'min': 1, 'max': 8, 'get_value': self.get_columns, 'on_change': self.on_menu_change, 'on_assert': self.set_grid_size}}})
    ...
    def set_grid_size(self):
        if libseq.getSequencesInBank(self.parent.bank) > self.parent.get_param("Grid size", "value") * self.parent.get_param("Grid size", "value"):
            self.zyngui.show_confirm("Reducing the quantity of sequences in bank %d will delete sequences but patterns will still be available. Continue?" % (
    ...
    def do_grid_size(self, params=None):
        ...
        for seq in range(libseq.getSequencesInBank(bank)):
            libseq.setPlayState(
                bank, seq, zynthian_gui_stepsequencer.SEQ_STOPPED)
        channels = []
        groups = []
        ...
        if delta < 0:
            ...
            libseq.setSequencesInBank(
                bank, new_size * self.columns)  # Lose excess columns
    ...
    def name_sequence(self, params=None):
        self.zyngui.show_keyboard(self.do_rename_sequence, zynseq.get_sequence_name(
            self.parent.bank, self.selected_pad), 16)
    ...
    def on_menu_change(self, params):
        ...
        if menu_item == 'Play mode':
            ...
            libseq.setChannel(self.parent.bank,
                              self.selected_pad, 0, value - 1)
    ...
    def update_grid(self):
        ...
        self.state_icon[zynthian_gui_stepsequencer.SEQ_STOPPED] = ImageTk.PhotoImage(
            img)
        img = (Image.open("/zynthian/zynthian-ui/icons/starting.png").resize(iconsize))
        self.state_icon[zynthian_gui_stepsequencer.SEQ_STARTING] = ImageTk.PhotoImage(
            img)
        img = (Image.open("/zynthian/zynthian-ui/icons/playing.png").resize(iconsize))
        self.state_icon[zynthian_gui_stepsequencer.SEQ_PLAYING] = ImageTk.PhotoImage(
            img)
        img = (Image.open("/zynthian/zynthian-ui/icons/stopping.png").resize(iconsize))
        self.state_icon[zynthian_gui_stepsequencer.SEQ_STOPPING] = ImageTk.PhotoImage(
            img)
        ...
        for pad in range(self.columns**2):
            ...
            self.grid_canvas.create_image(int(pad_x + self.column_width * 0.2), int(
                pad_y + 0.9 * self.row_height), tags=("mode:%d" % (pad), "trigger_%d" % (pad)), anchor="sw")
            self.grid_canvas.create_image(int(pad_x + self.column_width * 0.9), int(
                pad_y + 0.9 * self.row_height), tags=("state:%d" % (pad), "trigger_%d" % (pad)), anchor="se")
            self.grid_canvas.tag_bind("trigger_%d" % (
                pad), '<Button-1>', self.on_pad_press)
            self.grid_canvas.tag_bind("trigger_%d" % (
            ...
            if force or libseq.hasSequenceChanged(self.parent.bank, pad):
                ...
                if libseq.getSequenceLength(self.parent.bank, pad) == 0 or mode == zynthian_gui_stepsequencer.SEQ_DISABLED:
                    self.grid_canvas.itemconfig(
                        cell, fill=zynthian_gui_stepsequencer.PAD_COLOUR_DISABLED)
                else:
                    self.grid_canvas.itemconfig(
                        cell, fill=zynthian_gui_stepsequencer.PAD_COLOUR_STOPPED[group % 16])
                ...
                if libseq.getSequenceLength(self.parent.bank, pad) == 0:
                    mode = 0
                self.grid_canvas.itemconfig("lbl_pad:%d" % (
                    pad), text=zynseq.get_sequence_name(self.parent.bank, pad), fill=foreground)
                self.grid_canvas.itemconfig("group:%s" % (pad), text=chr(
                    65 + libseq.getGroup(self.parent.bank, pad)), fill=foreground)
                self.grid_canvas.itemconfig(
                    "mode:%d" % pad, image=self.mode_icon[mode])
                if state == 0 and libseq.isEmpty(self.parent.bank, pad):
                    self.grid_canvas.itemconfig(
                        "state:%d" % pad, image=self.empty_icon)
                else:
                    self.grid_canvas.itemconfig(
                        "state:%d" % pad, image=self.state_icon[state])
    ...
    def show_editor(self):
        ...
        patterns_in_track = libseq.getPatternsInTrack(
            self.parent.bank, self.selected_pad, 0)
