...
class zynthian_gui_patterneditor():
    ...
    def __init__(self, parent):
        ...
        # What to redraw: 0=nothing, 1=existing elements, 2=recreate grid
...
#		self.startPlayheadHandler()
    ...
    def show(self, params=None):
        try:
            ...
            self.title = "Pattern %d (Seq: %s)" % (
                params['pattern'], params['name'])
        except:
    ...
    def hide(self):
        ...
        self.parent.unregister_switch(
            zynthian_gui_stepsequencer.ENC_SELECT, "SB")
        self.parent.unregister_switch(
            zynthian_gui_stepsequencer.ENC_SNAPSHOT, "SB")
    ...
    def populate_menu(self):
        self.parent.add_menu({'Beats in pattern': {'method': self.parent.show_param_editor, 'params': {
                             'min': 1, 'max': 16, 'get_value': libseq.getBeatsInPattern, 'on_change': self.on_menu_change, 'on_assert': self.assert_beats_in_pattern}}})
        self.parent.add_menu({'Steps per beat': {'method': self.parent.show_param_editor, 'params': {'min': 0, 'max': len(
            STEPS_PER_BEAT)-1, 'get_value': self.get_steps_per_beat_index, 'on_change': self.on_menu_change, 'on_assert': self.assert_steps_per_beat}}})
        self.parent.add_menu({'-------------------': {}})
        self.parent.add_menu({'Copy pattern': {'method': self.parent.show_param_editor, 'params': {'min': 1, 'max': 64872,
                             'get_value': self.get_pattern, 'on_change': self.on_menu_change, 'on_assert': self.copy_pattern, 'on_cancel': self.cancel_copy}}})
        self.parent.add_menu({'Clear pattern': {'method': self.clear_pattern}})
        if libseq.getScale():
            self.parent.add_menu({'Transpose pattern': {'method': self.parent.show_param_editor, 'params': {
                                 'min': -1, 'max': 1, 'value': 0, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'-------------------': {}})
        self.parent.add_menu({'Vertical zoom': {'method': self.parent.show_param_editor, 'params': {
                             'min': 1, 'max': 127, 'get_value': self.get_vertical_zoom, 'on_change': self.on_menu_change, 'on_assert': self.assert_zoom}}})
        self.parent.add_menu({'Scale': {'method': self.parent.show_param_editor, 'params': {
                             'min': 0, 'max': self.get_scales(), 'get_value': libseq.getScale, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'Tonic': {'method': self.parent.show_param_editor, 'params': {
                             'min': -1, 'max': 12, 'get_value': libseq.getTonic, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'Rest note': {'method': self.parent.show_param_editor, 'params': {
                             'min': -1, 'max': 128, 'get_value': libseq.getInputRest, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'Input channel': {'method': self.parent.show_param_editor, 'params': {
                             'min': 0, 'max': 16, 'get_value': self.get_input_channel, 'on_change': self.on_menu_change}}})
    ...
    def export_smf(self, params):
        ...
        for step in range(libseq.getSteps()):
            ...
            for note in range(128):
                ...
                libsmf.addNote(smf, 0, time, duration,
                               self.channel, note, velocity)
        libsmf.setEndOfTrack(smf, 0, int(libseq.getSteps() * ticks_per_step))
        zynsmf.save(smf, "/zynthian/zynthian-my-data/capture/pattern%d_%s.mid" %
    ...
    def enable_edit(self, enable):
        if enable:
            ...
            self.parent.set_title(
                "Note Parameters", zynthian_gui_config.color_header_bg, zynthian_gui_config.color_panel_tx)
        else:
            self.edit_mode = False
            self.parent.unregister_switch(ENC_BACK)
            self.parent.set_title(
                self.title, zynthian_gui_config.color_panel_tx, zynthian_gui_config.color_header_bg)

    # Function to assert steps per beat

    def assert_steps_per_beat(self):
        self.zyngui.show_confirm(
            "Changing steps per beat may alter timing and/or lose notes?", self.do_steps_per_beat)

    # Function to actually change steps per beat

    def do_steps_per_beat(self, params=None):
        libseq.setStepsPerBeat(
            STEPS_PER_BEAT[self.parent.get_param('Steps per beat', 'value')])
    ...
    def assert_beats_in_pattern(self):
        ...
        if libseq.getLastStep() >= libseq.getStepsPerBeat() * value:
            self.zyngui.show_confirm(
                "Reducing beats in pattern will truncate pattern", self.set_beats_in_pattern)
        else:
            self.set_beats_in_pattern()

    # Function to assert beats in pattern
    def set_beats_in_pattern(self, params=None):
        libseq.setBeatsInPattern(
            self.parent.get_param('Beats in pattern', 'value'))
        self.redraw_pending = 2
            ...
            if scale == 0:
                ...
                for octave in range(0, 9):
                    for offset in data[scale]['scale']:
                        ...
                        if note > 127:
                            break
                        self.keymap.append({"note": note, "name": "%s%d" % (
                            self.notes[note % 12], note // 12 - 1)})
                        if note <= base_note:
                            self.keymap_offset = len(self.keymap) - 1
                            self.selected_cell[1] = self.keymap_offset
    ...
    def on_pianoroll_press(self, event):
        ...
        index = self.keymap_offset + self.zoom - \
            int(event.y / self.row_height) - 1
    ...
    def on_pianoroll_motion(self, event):
        if not self.piano_roll_drag_start:
            return
        offset = int((event.y - self.piano_roll_drag_start.y) /
                     self.row_height)
        if offset == 0:
            return
    ...
    def on_grid_drag(self, event):
        ...
        if self.drag_start_velocity:
            ...
            if self.drag_velocity:
                ...
                if value:
                    self.velocity = int(
                        self.drag_start_velocity + value * self.height / 100)
                    ...
                    self.velocity_canvas.coords(
                        "velocityIndicator", 0, 0, self.piano_roll_width * self.velocity / 127, PLAYHEAD_HEIGHT)
                    if libseq.getNoteDuration(self.selected_cell[0], note):
                        libseq.setNoteVelocity(
                            self.selected_cell[0], note, self.velocity)
            ...
            if self.drag_duration:
                ...
                if duration != self.duration and duration > 0:
                    ...
                    # Change length by adding event over previous one
                    self.add_event(step, index)
        else:
            # Clicked on empty cell so want to add a new note by dragging towards the desired cell
            x1 = self.selected_cell[0] * \
                self.step_width  # x pos of start of event
            # x pos right of event's first cell
            x3 = (self.selected_cell[0] + 1) * self.step_width
            # y pos of top of selected row
            y1 = self.grid_height - \
                (self.selected_cell[1] - self.keymap_offset) * self.row_height
            # y pos of bottom of selected row
            y2 = self.grid_height - \
                (self.selected_cell[1] -
                 self.keymap_offset + 1) * self.row_height
            if event.x < x1:
    ...
    def draw_cell(self, step, row):
        ...
        cellIndex = row * libseq.getSteps() + step
        ...
        if not duration:
            duration = 1
        fill_colour = "#%02x%02x%02x" % (
            velocity_colour, velocity_colour, velocity_colour)
        ...
        if cell:
            ...
            # Create new cell
            cell = self.grid_canvas.create_rectangle(coord, fill=fill_colour, width=0, tags=(
                "%d,%d" % (step, row), "gridcell", "step%d" % step))
            self.grid_canvas.tag_bind(
                cell, '<ButtonPress-1>', self.on_grid_press)
            self.grid_canvas.tag_bind(
                cell, '<ButtonRelease-1>', self.on_grid_release)
        ...
        if step + duration > libseq.getSteps():
            self.grid_canvas.itemconfig(
                "lastnotetext%d" % row, text="+%d" % (duration - libseq.getSteps() + step), state="normal")
    ...
    def draw_grid(self):
        ...
        if clear_grid:
            ...
            self.play_canvas.coords("playCursor", 1 + self.playhead * self.step_width,
                                    0, 1 + self.playhead * self.step_width + self.step_width, PLAYHEAD_HEIGHT)
        # Draw cells of grid
        ...
        if libseq.getStepsPerBeat():
            for step in range(0, libseq.getSteps() + 1, libseq.getStepsPerBeat()):
                self.grid_canvas.create_line(step * self.step_width, 0, step * self.step_width,
                                             self.zoom * self.row_height - 1, fill=GRID_LINE, tags=("gridline"))
        ...
        for row in range(0, self.zoom):
            ...
            if clear_grid:
                ...
                self.grid_canvas.create_text(self.grid_width - self.select_thickness, int(self.row_height * (
                    self.zoom - row - 0.5)), state="hidden", tags=("lastnotetext%d" % (row), "lastnotetext"), font=font, anchor="e")
            self.draw_row(index)
            ...
            if name:
                self.piano_roll.create_text((2, self.row_height * (self.zoom - row - 0.5)),
                                            text=name, font=font, anchor="w", fill=CANVAS_BACKGROUND, tags="notename")
            if self.keymap[index]['note'] % 12 == libseq.getTonic():
                self.grid_canvas.create_line(0, (self.zoom - row) * self.row_height, self.grid_width,
                                             (self.zoom - row) * self.row_height, fill=GRID_LINE, tags=("gridline"))
    ...
    def draw_pianoroll(self):
        ...
        for row in range(self.zoom):
            ...
            y2 = y1 + self.row_height - 1
            id = "row%d" % (row)
            id = self.piano_roll.create_rectangle(
                x1, y1, x2, y2, width=0, tags=id)
    ...
    def clear_pattern(self, params=None):
        self.zyngui.show_confirm("Clear pattern %d?" %
                                 (self.pattern), self.do_clear_pattern)
    ...
    def do_clear_pattern(self, params=None):
        ...
        if libseq.getPlayState(self.bank, self.sequence, 0) != zynthian_gui_stepsequencer.SEQ_STOPPED:
            libseq.sendMidiCommand(0xB0 | self.channel,
                                   123, 0)  # All notes off
    ...
    def copy_pattern(self):
        if libseq.getLastStep() == -1:
            ...
            self.zyngui.show_confirm("Overwrite pattern %d with content from pattern %d?" % (
                self.pattern, self.copy_source), self.do_copy_pattern, self.pattern)
    ...
    def do_copy_pattern(self, dest_pattern):
        ...
        self.parent.arranger.pattern_canvas.itemconfig(
            "patternIndicator", text="%d" % (self.pattern))
    ...
    def on_zyncoder(self, encoder, value):
        if encoder == ENC_BACK:
            if self.edit_mode:
                ...
                self.parent.set_title("Velocity: %d" %
                                      (self.velocity), None, None, 2)
            else:
            ...
            if self.edit_mode:
                ...
                if libseq.getNoteDuration(self.selected_cell[0], note):
                    ...
                    self.select_cell()
                self.parent.set_title("Duration: %d steps" %
                                      (self.duration), None, None, 2)
            else:
