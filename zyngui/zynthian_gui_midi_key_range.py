...
class zynthian_gui_midi_key_range(zynthian_gui_base):
    ...
    def __init__(self):
        ...
        self.space_frame_width = zynthian_gui_config.display_width - \
            2*zynthian_gui_config.ctrl_width
        ...
        self.space_frame.grid(row=1, column=1, rowspan=1, columnspan=1, padx=(
            2, 2), pady=(0, 2), sticky="wens")
        ...
        self.note_info_canvas = tkinter.Canvas(self.main_frame,
                                               ...
                                               relief='flat',
                                               bg=zynthian_gui_config.color_panel_bg)
        self.note_info_canvas.grid(
            row=2, column=0, rowspan=1, columnspan=3, sticky="wens")
        ...
        self.piano_canvas = tkinter.Canvas(self.main_frame,
                                           ...
                                           relief='flat',
                                           bg="#000000")
        self.piano_canvas.grid(row=3, column=0, rowspan=1,
    ...
    def config(self, chan):
        ...
        self.octave_trans = zyncoder.lib_zyncoder.get_midi_filter_octave_trans(
            chan)
        self.halftone_trans = zyncoder.lib_zyncoder.get_midi_filter_halftone_trans(
            chan)
    ...
    def update_text(self):
        self.note_info_canvas.itemconfig(
    ...
    def set_zctrls(self):
        if self.shown:
            if self.nlow_zctrl:
                ...
                self.nlow_ctrl = zynthian_controller(None, 'note_low', 'note_low', {
                                                     'midi_cc': 0, 'value_max': 127})
            ...
            if self.nhigh_zctrl:
                ...
                self.nhigh_ctrl = zynthian_controller(None, 'note_high', 'note_high', {
                                                      'midi_cc': 0, 'value_max': 127})
            ...
            if self.octave_zctrl:
                ...
                self.octave_ctrl = zynthian_controller(
                    None, 'octave transpose', 'octave transpose', {'midi_cc': 0, 'value_max': 11})
            ...
            if self.halftone_zctrl:
                ...
                self.halftone_ctrl = zynthian_controller(
                    None, 'semitone transpose', 'semitone transpose', {'midi_cc': 0, 'value_max': 25})
        ...
        if self.shown:
            ...
            if self.note_low != self.nlow_zctrl.value:
                if self.nlow_zctrl.value > self.note_high:
                    ...
                    logging.debug(
                        "SETTING FILTER NOTE_LOW: {}".format(self.note_low))
                    zyncoder.lib_zyncoder.set_midi_filter_note_low(
                        self.chan, self.note_low)
                    self.replot = True
            ...
            if self.note_high != self.nhigh_zctrl.value:
                ...
                logging.debug(
                    "SETTING FILTER NOTE_HIGH: {}".format(self.note_high))
                zyncoder.lib_zyncoder.set_midi_filter_note_high(
                    self.chan, self.note_high)
                self.replot = True
            ...
            if (self.octave_trans+5) != self.octave_zctrl.value:
                ...
                logging.debug(
                    "SETTING FILTER OCTAVE TRANS.: {}".format(self.octave_trans))
                zyncoder.lib_zyncoder.set_midi_filter_octave_trans(
                    self.chan, self.octave_trans)
                self.replot = True
            ...
            if (self.halftone_trans+12) != self.halftone_zctrl.value:
                ...
                logging.debug("SETTING FILTER HALFTONE TRANS.: {}".format(
                    self.halftone_trans))
                zyncoder.lib_zyncoder.set_midi_filter_halftone_trans(
                    self.chan, self.halftone_trans)
                self.replot = True
    ...
    def set_select_path(self):
        try:
            self.select_path.set("{} > Note Range & Transpose...".format(
                self.zyngui.screens['layer_options'].layer.get_basepath()))
        except:
