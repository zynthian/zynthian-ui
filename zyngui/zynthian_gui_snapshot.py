...
class zynthian_gui_snapshot(zynthian_gui_selector):
    ...
    def get_next_name(self):
        n = max(map(lambda item: int(item[2].split(
            '-')[0]) if item[2].split('-')[0].isdigit() else 0, self.list_data))
        return "{0:03d}".format(n+1)

    def get_new_snapshot(self):
        parts = self.zyngui.screens['layer'].layers[0].get_presetpath().split(
            '#', 2)
    ...
    def check_bankless_mode(self):
        banks = [d for d in os.listdir(self.base_dir) if os.path.isdir(
    ...
    def load_bank_list(self):
        ...
        if isfile(self.last_state_snapshot_fpath):
            self.list_data.append(
                (self.last_state_snapshot_fpath, i, "Last State"))
            i = i+1
        ...
        for f in sorted(os.listdir(self.base_dir)):
            ...
            if isdir(dpath):
                ...
                try:
                    ...
                    logging.debug(
                        "Snapshot Bank '%s' => MIDI bank %d" % (f, bn))
                except:
                    logging.warning(
                        "Snapshot Bank '%s' don't have a MIDI bank number" % f)
                i = i+1
    ...
    def load_snapshot_list(self):
        ...
        if self.bankless_mode:
            if isfile(self.default_snapshot_fpath):
                self.list_data.append(
                    (self.default_snapshot_fpath, i, "Default"))
                i += 1
            if isfile(self.last_state_snapshot_fpath):
                self.list_data.append(
                    (self.last_state_snapshot_fpath, i, "Last State"))
        ...
        for f in sorted(os.listdir(join(self.base_dir, self.bank_dir))):
            ...
            if isfile(fpath) and f[-4:].lower() == '.zss':
                ...
                try:
                    ...
                    logging.debug(
                        "Snapshot '{}' => MIDI bank {}, program {}".format(title, bn, pn))
                except:
                    logging.warning(
                        "Snapshot '{}' don't have a MIDI program number".format(title))
                i += 1
            ...
            if fpath:
                if fpath == "SAVE":
                    ...
                    self.zyngui.show_keyboard(
                        self.save_snapshot_by_name, parts[1])
                else:
                    self.show_options(i)
    ...
    def show_options(self, i):
        ...
        options = {"Load": fpath, "Save": fname, "Rename": fname,
                   "Create copy": fname, "Set program": fname, "Delete": fname}
    ...
    def options_cb(self, option, param):
        ...
        if parts is None:
            logging.warning(
                "Wrong snapshot {} => {}".format(self.index, fpath))
            return

        if option == "Load":
            self.zyngui.screens['layer'].load_snapshot(fpath)
        elif option == "Save":
            self.zyngui.show_confirm("Do you really want to overwrite %s with current configuration" % (
                fname), self.save_snapshot, fpath)
        elif option == "Rename":
            ...
            self.zyngui.show_confirm("Do you really want to delete %s" % (
                fname), self.delete_confirmed, self.get_snapshot_fpath(fpath))
    ...
    def rename_snapshot(self, new_name):
        ...
        if new_path[-4:].lower() != '.zss':
            new_path += '.zss'
        if isfile(new_path):
            self.zyngui.show_confirm("Do you really want to overwrite the snapshot %s?" %
                                     new_name, self.do_rename, [parts[3], new_path])
        else:
    ...
    def do_rename(self, data):
        try:
            ...
            logging.warning(
                "Failed to rename snapshot {} to {} => {}".format(data[0], data[1], e))
    ...
    def copy_snapshot(self, new_name):
        ...
        if isfile(new_path):
            self.zyngui.show_confirm("Do you really want to overwrite the snapshot %s?" %
                                     new_name, self.do_copy, [parts[3], new_path])
        else:
    ...
    def do_copy(self, data):
        try:
            ...
            logging.warning(
                "Failed to copy snapshot {} to {} => {}".format(data[0], data[1], e))
    ...
    def set_program(self, value):
        ...
        try:
            ...
            logging.warning(
                "Failed to set program for snapshot {} to {} => {}".format(fpath, program, e))
    ...
    def save_last_state_snapshot(self):
        self.zyngui.screens['layer'].save_snapshot(
    ...
    def midi_bank_change_offset(self, offset):
        try:
            ...
            logging.warning(
                "Can't do Snapshot Bank Change Offset {}".format(offset))
