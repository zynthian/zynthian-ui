...
class zynthian_gui_zs3_learn(zynthian_gui_selector):
    ...
    def fill_list(self):
        ...
        try:
            ...
            zs3_indexes = self.zyngui.screens['layer'].get_midi_chan_zs3_used_indexes(
                midich)
            select_zs3_idx = self.zyngui.screens['layer'].get_last_zs3_index(
                midich)
            self.num_programs = len(zs3_indexes)
            for i, zs3_index in enumerate(zs3_indexes):
                zs3_title = "Program {}".format(zs3_index)
                self.list_data.append(
                    (zs3_index, len(self.list_data), zs3_title))
        ...
        self.list_data.append(
            ('None', len(self.list_data), "Waiting for Program Change ..."))
    ...
    def set_selector(self):
        if self.zselector:
            self.zselector_ctrl.set_options({'symbol': self.selector_caption, 'name': self.selector_caption,
                                            'short_name': self.selector_caption, 'midi_cc': 0, 'value_max': self.num_programs, 'value': self.index})
            self.zselector.config(self.zselector_ctrl)
            self.zselector.show()
        else:
            self.zselector_ctrl = zynthian_controller(None, self.selector_caption, self.selector_caption, {
                                                      'midi_cc': 0, 'value_max': self.num_programs, 'value': self.index})
            self.zselector = zynthian_gui_controller(
                zynthian_gui_config.select_ctrl, self.main_frame, self.zselector_ctrl)
    ...
    def select_action(self, i, t='S'):
        ...
        if isinstance(zs3_index, int):
            midich = self.zyngui.curlayer.get_midi_chan()
            if t == 'S':
                self.zyngui.screens['layer'].set_midi_chan_zs3(
                    midich, zs3_index)
    ...
    def set_select_path(self):
        if self.zyngui.curlayer:
            self.select_path.set(
                self.zyngui.curlayer.get_basepath() + " /PROG MIDI-Learn")
        else:
