...
class zynthian_gui_audio_recorder(zynthian_gui_selector):
    ...
    def __init__(self):
        ...
        self.capture_dir_usb = os.environ.get(
            'ZYNTHIAN_EX_DATA_DIR', "/media/usb0")
    ...
    def fill_list(self):
        ...
        for fname, finfo in self.get_filelist(self.capture_dir_sdc).items():
            l = finfo['length']
            title = "SD[{}:{:02d}] {}".format(
                int(l/60), int(l % 60), fname.replace(";", ">", 1).replace(";", "/"))
    ...
    def get_filelist(self, src_dir):
        ...
        for fname in res:
            try:
                res[fname]['length'] = mutagen.File(
                    res[fname]['fpath']).info.length
        ...
        for i, row in enumerate(self.list_data):
            if row[0] is not None and row[0] == self.current_playback_fpath:
                self.listbox.itemconfig(
                    i, {'bg': zynthian_gui_config.color_hl})
            else:
                self.listbox.itemconfig(
    ...
    def get_next_filenum(self):
        try:
            n = max(map(lambda item: int(os.path.basename(item[0])[
                    0:3]) if item[0] and os.path.basename(item[0])[0:3].isdigit() else 0, self.list_data))
        except:
            n = 0
            ...
            try:
                ...
                if check_mount.wait() != 0:
                    ...
                    try:
                        ...
                        pass
                self.rec_proc = Popen(
                    ("/usr/local/bin/jack_capture", "--daemon", self.get_new_filename()), cwd=capture_dir)
            except Exception as e:
    ...
    def start_playing(self, fpath=None):
        ...
        try:
            ...
            try:
                mplayer_options += " -ao jack:port=\"{}\"".format(
                    self.audio_out[0])
            except:
                mplayer_options += " -ao jack"

            mplayer_options += " -input file=\"{}\"".format(
                self.mplayer_ctrl_fifo_path)
            cmd = "/usr/bin/mplayer {} \"{}\"".format(mplayer_options, fpath)

            logging.info("COMMAND: %s" % cmd)

            def runInThread(onExit, cmd):
                self.play_proc = Popen(
            ...
            thread = threading.Thread(target=runInThread, args=(
                self.end_playing, cmd), daemon=True)
            thread.start()
            sleep(0.5)
    ...
    def show_playing_volume(self):
        if self.volume_zgui_ctrl:
            ...
            self.volume_zgui_ctrl = zynthian_gui_controller(
                2, self.main_frame, self.volume_zctrl)
    ...
    def toggle_loop(self):
        ...
        zynconf.save_config({"ZYNTHIAN_AUDIO_PLAY_LOOP": str(
            int(zynthian_gui_config.audio_play_loop))})
        self.update_list()
