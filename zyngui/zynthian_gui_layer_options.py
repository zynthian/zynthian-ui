...
class zynthian_gui_layer_options(zynthian_gui_selector):
    ...
    def fill_list(self):
        ...
        if self.audiofx_layer:
            if len(self.audiofx_layer.preset_list) > 1:
                self.list_data.append(
                    (self.audiofx_presets, None, "Audio-FX Presets"))

            self.list_data.append(
                (self.audiofx_replace, None, "Replace Audio-FX"))

            if self.audiofx_can_move_upchain():
                self.list_data.append(
                    (self.audiofx_move_upchain, None, "Move Upchain"))

            if self.audiofx_can_move_downchain():
                self.list_data.append(
                    (self.audiofx_move_downchain, None, "Move Downchain"))

            self.list_data.append(
                (self.audiofx_remove, None, "Remove Audio-FX"))

        elif self.midifx_layer:
            if len(self.midifx_layer.preset_list) > 1:
                self.list_data.append(
                    (self.midifx_presets, None, "MIDI-FX Presets"))

            self.list_data.append(
                (self.midifx_replace, None, "Replace MIDI-FX"))

            if self.midifx_can_move_upchain():
                self.list_data.append(
                    (self.midifx_move_upchain, None, "Move Upchain"))

            if self.midifx_can_move_downchain():
                self.list_data.append(
                    (self.midifx_move_downchain, None, "Move Downchain"))
        ...
        # Root Layer Options
            ...
            self.audiofx_layers = self.zyngui.screens['layer'].get_fxchain_layers(
                self.layer)
            if self.audiofx_layers and len(self.audiofx_layers) > 0:
                self.audiofx_layers.remove(self.layer)

            self.midifx_layers = self.zyngui.screens['layer'].get_midichain_layers(
                self.layer)
            ...
            if self.layer.engine.type != 'MIDI Tool':
                ...
                if len(self.audiofx_layers) > 0:
                    self.list_data.append(
                        (self.audiofx_reset, None, "Remove All Audio-FX"))
                    # Add Audio-FX layers
                    sl0 = None
                    for sl in self.audiofx_layers:
                        if sl.is_parallel_audio_routed(sl0):
                            ...
                            bullet = " -> "
                        self.list_data.append(
                            (self.audiofx_layer_action, sl, bullet + sl.engine.get_path(sl)))
                        sl0 = sl
            ...
            if self.layer.engine.type in ('MIDI Synth', 'MIDI Tool', 'Special') and self.layer.engine.nickname != 'MD':
                ...
                if self.layer.midi_chan is not None:
                    self.list_data.append(
                        (self.midifx_add, None, "Add MIDI-FX"))

                if len(self.midifx_layers) > 0:
                    self.list_data.append(
                        (self.midifx_reset, None, "Remove All MIDI-FX"))
                    # Add MIDI-FX layers
                    sl0 = None
                    for sl in self.midifx_layers:
                        if sl.is_parallel_midi_routed(sl0):
                            ...
                            bullet = " -> "
                        self.list_data.append(
                            (self.midifx_layer_action, sl, bullet + sl.engine.get_path(sl)))
                        sl0 = sl
    ...
    def audiofx_layer_action(self, layer, t='S'):
        ...
        self.audiofx_layer_index = self.zyngui.screens['layer'].layers.index(
            layer)
        self.show()
    ...
    def midifx_layer_action(self, layer, t='S'):
        ...
        self.midifx_layer_index = self.zyngui.screens['layer'].layers.index(
            layer)
        self.show()
    ...
    def layer_midi_chan(self):
        chan_list = self.zyngui.screens['layer'].get_free_midi_chans(
        ) + [self.layer.midi_chan]
        chan_list.sort()
        self.zyngui.screens['midi_chan'].set_mode(
            "SET", self.layer.midi_chan, chan_list)
        self.zyngui.show_modal('midi_chan')

    def layer_clone(self):
        self.zyngui.screens['midi_chan'].set_mode(
            "CLONE", self.layer.midi_chan)
        self.zyngui.show_modal('midi_chan')
    ...
    def layer_remove(self):
        self.zyngui.show_confirm(
            "Do you really want to remove this layer?", self.layer_remove_confirmed)
    ...
    def layer_midi_unlearn(self):
        self.zyngui.show_confirm(
            "Do you really want to clean MIDI-learn for this layer?", self.layer_midi_unlearn_confirmed)
    ...
    def audiofx_reset(self):
        self.zyngui.show_confirm(
            "Do you really want to remove all audio-FXs for this layer?", self.audiofx_reset_confirmed)
    ...
    def audiofx_can_move_upchain(self):
        ups = self.zyngui.screens['layer'].get_fxchain_upstream(
            self.audiofx_layer)
    ...
    def audiofx_can_move_downchain(self):
        downs = self.zyngui.screens['layer'].get_fxchain_downstream(
            self.audiofx_layer)
    ...
    def audiofx_replace(self):
        midi_chan = self.layer.midi_chan
        self.zyngui.screens['layer'].replace_fxchain_layer(
            self.audiofx_layer_index)
    ...
    def midifx_reset(self):
        self.zyngui.show_confirm(
            "Do you really want to remove all MIDI-FXs for this layer?", self.midifx_reset_confirmed)
    ...
    def midifx_can_move_upchain(self):
        ups = self.zyngui.screens['layer'].get_midichain_upstream(
            self.midifx_layer)
        if len(ups) > 0 and self.layer not in ups:
            return True
    ...
    def midifx_can_move_downchain(self):
        downs = self.zyngui.screens['layer'].get_midichain_downstream(
            self.midifx_layer)
        if len(downs) > 0 and self.layer not in downs:
            return True
    ...
    def midifx_move_downchain(self):
        downs = self.zyngui.screens['layer'].get_midichain_downstream(
            self.midifx_layer)
        self.zyngui.screens['layer'].swap_midichain(
            self.midifx_layer, downs[0])
        self.back_action()

    def midifx_replace(self):
        midi_chan = self.layer.midi_chan
        self.zyngui.screens['layer'].replace_midichain_layer(
            self.midifx_layer_index)
    ...
    def set_select_path(self):
        if self.audiofx_layer:
            self.select_path.set("{} > Options".format(
                self.audiofx_layer.get_basepath()))
        elif self.audiofx_layer:
            self.select_path.set("{} > Options".format(
                self.midifx_layer.get_basepath()))
        elif self.layer:
            self.select_path.set(
                "{} > Options".format(self.layer.get_basepath()))
        else:
