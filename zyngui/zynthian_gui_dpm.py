...
class zynthian_gui_dpm():
	...
	def __init__(self, zynmixer, strip, channel, parent, x0, y0, width, height, vertical=True, tags=()):
		...
		if self.vertical:
			...
			self.bg_over = self.parent.create_rectangle(self.x0, self.y0, self.x1, self.y_over, width=0, fill=self.over_color, tags=tags)
			self.bg_high = self.parent.create_rectangle(self.x0, self.y_over, self.x1, self.y_high, width=0, fill=self.high_color, tags=tags)
			self.bg_low = self.parent.create_rectangle(self.x0, self.y_high, self.x1, self.y_low, width=0, fill=self.low_color, tags=tags)
			self.overlay = self.parent.create_rectangle(self.x0, self.y0, self.x1, self.y1, width=0, fill=self.bg_color, tags=tags)
			self.hold = self.parent.create_rectangle(self.x0, self.y_low, self.x1, self.y_low, width=0, fill=self.low_color, tags=tags, state=HIDDEN)
			self.parent.create_line(self.x0, y_zero, self.x1, y_zero, fill=self.line_color, tags=tags)
			...
			self.bg_over = self.parent.create_rectangle(self.x_over, self.y0, self.x1, self.y1, width=0, fill=self.over_color, tags=tags)
			self.bg_high = self.parent.create_rectangle(self.x_high, self.y0, self.x_over, self.y1, width=0, fill=self.high_color, tags=tags)
			self.bg_low = self.parent.create_rectangle(self.x_low, self.y0, self.x_high, self.y1, width=0, fill=self.low_color, tags=tags)
			self.overlay = self.parent.create_rectangle(self.x0, self.y0, self.x1, self.y1, width=0, fill=self.bg_color, tags=tags)
			self.hold = self.parent.create_rectangle(self.x_low, self.y0, self.x_low, self.y1, width=0, fill=self.low_color, tags=tags, state=HIDDEN)
			self.parent.create_line(x_zero, self.y0, x_zero, self.y1, fill=self.line_color, tags=tags)
	...
	def refresh(self, dpm, hold, mono):
		...
		if self.vertical:
			...
			if y1 <= self.y_over:
				...
				if self.mono:
					self.parent.itemconfig(self.hold, state=NORMAL, fill="#FFFFFF")
				else:
					self.parent.itemconfig(self.hold, state=NORMAL, fill="#00FF00")
			else:
