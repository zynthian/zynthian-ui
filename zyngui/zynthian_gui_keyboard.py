...
class zynthian_gui_keyboard():
    ...
    def add_button(self, label, col, row, colspan=1):
        ...
        l = self.key_canvas.create_text(1 + self.key_width * (col + colspan / 2),
                                        1 + self.key_height * (row + 0.5), text=label, fill="white", tags=(tag))
    ...
    def setup_encoders(self):
        if self.zyncoder:
            ...
            self.zyncoder.setup_zyncoder(
                ENC_SELECT, pin_a, pin_b, 0, 0, None, 64, 127, 0)
            pin_a = zynthian_gui_config.zyncoder_pin_a[ENC_BACK]
            pin_b = zynthian_gui_config.zyncoder_pin_b[ENC_BACK]
            self.zyncoder.setup_zyncoder(
                ENC_BACK, pin_a, pin_b, 0, 0, None, 64, 127, 0)
    ...
    def on_zyncoder(self, encoder, value):
        if encoder == ENC_SELECT:
            ...
            selection = self.selected_button + value * \
                int(len(self.keys) / (self.rows - 1))
            if self.mode == OSK_NUMPAD:
    ...
    def back_up(self):
        if self.zyncoder:
            self.zyncoder.set_value_zyncoder(
                ENC_BACK, self.zyncoder.get_value_zyncoder(ENC_BACK) + 1)
    ...
    def select_up(self):
        if self.zyncoder:
            self.zyncoder.set_value_zyncoder(
                ENC_SELECT, self.zyncoder.get_value_zyncoder(ENC_SELECT) + 1)
