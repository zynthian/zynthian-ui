...
if os.environ.get('ZYNTHIAN_WIRING_ENCODER_A'):
    zyncoder_pin_a = list(map(int, os.environ.get(
        'ZYNTHIAN_WIRING_ENCODER_A').split(',')))
else:
    zyncoder_pin_a = None

if os.environ.get('ZYNTHIAN_WIRING_ENCODER_B'):
    zyncoder_pin_b = list(map(int, os.environ.get(
        'ZYNTHIAN_WIRING_ENCODER_B').split(',')))
else:
    zyncoder_pin_b = None

if os.environ.get('ZYNTHIAN_WIRING_SWITCHES'):
    zynswitch_pin = list(map(int, os.environ.get(
        'ZYNTHIAN_WIRING_SWITCHES').split(',')))
else:
    zynswitch_pin = None
...
if wiring_layout == "PROTOTYPE-1":
    ...
    select_ctrl = 3
    # Default to DUMMIES
    if not zyncoder_pin_a:
        zyncoder_pin_a = [0, 0, 0, 0]
    if not zyncoder_pin_b:
        zyncoder_pin_b = [0, 0, 0, 0]
    if not zynswitch_pin:
...
enable_onscreen_buttons = int(os.environ.get(
    'ZYNTHIAN_UI_ONSCREEN_BUTTONS', False))
...
restore_last_state = int(os.environ.get(
    'ZYNTHIAN_UI_RESTORE_LAST_STATE', False))
snapshot_mixer_settings = int(os.environ.get(
    'ZYNTHIAN_UI_SNAPSHOT_MIXER_SETTINGS', False))
...
def set_midi_config():
    ...
    midi_single_active_channel = int(os.environ.get(
