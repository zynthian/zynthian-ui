...
class zynthian_widget_clippy(zynthian_widget_base.zynthian_widget_base):
    ...
    def on_canvas_drag(self, event):
        ...
        if self.drag_marker and self.frames:
            ...
            f = self.width / self.frames * self.zoom
            if self.drag_marker.startswith("crop_start"):
                self.widget_canvas.coords(self.crop_start_rect, 0, 0, event.x, h)
            elif self.drag_marker.startswith("crop_end"):
                self.widget_canvas.coords(self.crop_end_rect, event.x, 0, self.width, h)

    # Function to handle end of mouse drag
    def on_canvas_release(self, event):
        if self.drag_marker in self.processor.controllers_dict:
            self.processor.controllers_dict[self.drag_marker].set_value(self.drag_value)
