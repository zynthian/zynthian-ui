...
class zynthian_gui_layer(zynthian_gui_selector):
    ...
    def __init__(self):
        ...
        # Last selected ZS3 snapshot, per MIDI channel
        self.last_zs3_index = [0] * 16
    ...
    def fill_list(self):
        ...
        if len(self.root_layers) > 0:
            self.list_data.append(
                (None, len(self.list_data), "-----------------------------"))

        # Add fixed entries
        self.list_data.append(
            ('NEW_SYNTH', len(self.list_data), "NEW Synth Layer"))
        self.list_data.append(('NEW_AUDIO_FX', len(
            self.list_data), "NEW Audio-FX Layer"))
        self.list_data.append(
            ('NEW_MIDI_FX', len(self.list_data), "NEW MIDI-FX Layer"))
        self.list_data.append(('NEW_GENERATOR', len(
            self.list_data), "NEW Generator Layer"))
        self.list_data.append(
            ('NEW_SPECIAL', len(self.list_data), "NEW Special Layer"))
        self.list_data.append(
            ('RESET', len(self.list_data), "REMOVE All Layers"))
        self.list_data.append(
            (None, len(self.list_data), "-----------------------------"))
        self.list_data.append(
            ('ALL_OFF', len(self.list_data), "PANIC! All Notes Off"))
    ...
    def select_action(self, i, t='S'):
        ...
        if self.list_data[i][0] is None:
            ...
            self.zyngui.show_confirm(
                "Do you really want to remove all layers?", self.reset_confirmed)
    ...
    def show_chain_options_modal(self):
        ...
        self.zyngui.screens['option'].config(
            "Chain Mode", chain_modes, self.cb_chain_options_modal)
    ...
    def replace_fxchain_layer(self, i):
        ...
        self.layer_chain_parallel = False
        self.zyngui.screens['engine'].set_fxchain_mode(
            self.layers[i].midi_chan)
        self.zyngui.show_modal('engine')
    ...
    def replace_midichain_layer(self, i):
        ...
        self.replace_layer_index = i
        self.layer_chain_parallel = False
        self.zyngui.screens['engine'].set_midichain_mode(
            self.layers[i].midi_chan)
        self.zyngui.show_modal('engine')
    ...
    def add_layer_engine(self, eng, midi_chan=None):
        ...
        if eng == 'MD':
            ...
            self.zyngui.screens['midi_chan'].set_mode(
                "ADD", 0, self.get_free_midi_chans())
    ...
    def remove_layer(self, i, stop_unused_engines=True):
        if i >= 0 and i < len(self.layers):
            logging.debug("Removing layer {} => {} ...".format(
                i, self.layers[i].get_basepath()))
    ...
    def remove_root_layer(self, i, stop_unused_engines=True):
        if i >= 0 and i < len(self.root_layers):
            ...
            if self.root_layers[i].engine.nickname in ['BF', 'AE']:
                root_layers_to_delete = copy.copy(
                    self.root_layers[i].engine.layers)
            else:
            ...
            for root_layer in root_layers_to_delete:
                ...
                for layer in reversed(midichain_layers):
                    logging.debug("Mute MIDI layer '{}' ...".format(
                        i, layer.get_basepath()))
                ...
                for layer in reversed(fxchain_layers):
                    logging.debug("Mute Audio layer '{}' ...".format(
                        i, layer.get_basepath()))
    ...
    def remove_all_layers(self, stop_engines=True):
        ...
        while i > 0:
            i -= 1
            logging.debug("Mute layer {} => {} ...".format(
                i, self.layers[i].get_basepath()))
            self.drop_from_midichain(self.layers[i])
            self.layers[i].mute_midi_out()
            self.drop_from_fxchain(self.layers[i])
        ...
        while i > 0:
            i -= 1
            logging.debug("Remove layer {} => {} ...".format(
                i, self.layers[i].get_basepath()))
            self.layers[i].reset()
            self.layers.pop(i)
        self.zyngui.zynautoconnect_release_lock()
        ...
        for i in range(0, 16):
            for j in range(0, 16):
                if isinstance(clone_status[i][j], dict):
                    zyncoder.lib_zyncoder.set_midi_filter_clone(
                        i, j, clone_status[i][j]['enabled'])
                    self.zyngui.screens['midi_cc'].set_clone_cc(
                        i, j, clone_status[i][j]['cc'])
                else:
                    zyncoder.lib_zyncoder.set_midi_filter_clone(
                        i, j, clone_status[i][j])
    ...
    def set_transpose(self, tr_status):
        for i in range(0, 16):
            zyncoder.lib_zyncoder.set_midi_filter_halftone_trans(
                i, tr_status[i])

    def set_note_range(self, nr_status):
        for i in range(0, 16):
            zyncoder.lib_zyncoder.set_midi_filter_note_range(
                i, nr_status[i]['note_low'], nr_status[i]['note_high'], nr_status[i]['octave_trans'], nr_status[i]['halftone_trans'])
    ...
    def add_to_fxchain(self, layer, chain_parallel=False):
        try:
            for end in self.get_fxchain_ends(layer):
                if end != layer:
                    logging.debug("Adding to FX-chain {} => {}".format(
                        end.get_audio_jackname(), layer.get_audio_jackname()))
    ...
    def replace_on_fxchain(self, layer):
        try:
            ...
            logging.debug(
                "Replacing on FX-chain {} => {}".format(rlayer.get_jackname(), layer.get_jackname()))
    ...
    def drop_from_fxchain(self, layer):
        try:
            for up in self.get_fxchain_upstream(layer):
                logging.debug(
                    "Dropping from FX-chain {} => {}".format(up.get_jackname(), layer.get_jackname()))
    ...
    def add_to_midichain(self, layer, chain_parallel=False):
        try:
            for end in self.get_midichain_ends(layer):
                if end != layer:
                    logging.debug("Adding to MIDI-chain {} => {}".format(
                        end.get_midi_jackname(), layer.get_midi_jackname()))
    ...
    def replace_on_midichain(self, layer):
        try:
            rlayer = self.layers[self.replace_layer_index]
            logging.debug("Replacing on MIDI-chain {} => {}".format(
                rlayer.get_midi_jackname(), layer.get_midi_jackname()))
    ...
    def drop_from_midichain(self, layer):
        try:
            for up in self.get_midichain_upstream(layer):
                logging.debug("Dropping from MIDI-chain {} => {}".format(
                    up.get_midi_jackname(), layer.get_midi_jackname()))
    ...
    def save_snapshot(self, fpath):
        try:
            ...
            if 'stepseq' in self.zyngui.screens:
                ...
                b64_data = base64_encoded_data = base64.b64encode(
                    binary_riff_data)
            ...
            for i, lss in enumerate(snapshot['layers']):
                if lss['engine_nick'] == "MX":
                    ...
                    engine = self.zyngui.screens['engine'].start_engine(
                        lss['engine_nick'])
                    self.layers.append(zynthian_layer(
                        engine, lss['midi_chan'], self.zyngui))
