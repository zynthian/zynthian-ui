...
class zynthian_ctrldev_base():
	...
	def setup(self, idev):
		...
		if idev > 0:
			# Get the device ID string for the slot idev
			dev_id = zynautoconnect.get_midi_in_devid(idev)
			# If it's already active, don't setup again
			if idev == self.idev:
				return dev_id
			# Check if dev_id matches some driver in the list
			if self.dev_ids and dev_id in self.dev_ids:
				...
				# Check if device ID matches some device in the output device (feedback) list
				try:
					self.idev_out = zynautoconnect.devices_out.index(self.dev_id) + 1
				except:
					self.idev_out = 0
