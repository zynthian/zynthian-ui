...
class zynthian_gui_autoeq():
    ...
    def __init__(self):
        ...
        for i in range(self.n_bands):
            self.mon_bars.append(self.canvas.create_rectangle(
                x0, self.height, x0 + self.bar_width, self.height, fill=zynthian_gui_config.color_on))
            self.eq_bars.append(self.canvas.create_line(
                x0, self.height, x0 + self.bar_width, self.height, fill=zynthian_gui_config.color_hl))
            x0 += self.bar_width
    ...
    def refresh_bars(self):
        self.mon_layer = self.zyngui.screens['layer'].get_layer_by_jackname(
            '1/3')
        x0 = 0
        if self.mon_layer:
            i = 0
            for k, v in self.mon_layer.engine.get_lv2_monitors_dict().items():
                ...
                self.canvas.coords(
                    self.mon_bars[i], x0, self.height, x0 + self.bar_width, self.height - mon_y)
                x0 += self.bar_width
                i += 1
                if i >= self.n_bands:
                    break
            ...
            for i in range(self.n_bands):
                self.canvas.coords(
                    self.mon_bars[i], x0, self.height, x0 + self.bar_width, self.height)
                x0 += self.bar_width

        self.eq_layer = self.zyngui.screens['layer'].get_layer_by_jackname(
            'ZamGEQ31-01')
        x0 = 0
        if self.eq_layer:
            # TODO: To implement!!
            ...
            for i in range(self.n_bands):
                self.canvas.coords(
                    self.eq_bars[i], x0, 60, x0 + self.bar_width, 60)
                x0 += self.bar_width
