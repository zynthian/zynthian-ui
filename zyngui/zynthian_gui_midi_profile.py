...
class zynthian_gui_midi_profile(zynthian_gui_selector):
    ...
    def __init__(self):
        self.midi_profiles_dir = os.environ.get(
            'ZYNTHIAN_CONFIG_DIR', "/zynthian/config") + "/midi-profiles"
    ...
    def fill_list(self):
        self.current_profile = os.environ.get(
            "ZYNTHIAN_SCRIPT_MIDI_PROFILE", self.midi_profiles_dir + "/default.sh")
        self.list_data = []
        i = 0
    ...
    def select_action(self, i, t='S'):
        ...
        if profile_fpath == self.current_profile:
            logging.info(
                "MIDI PROFILE '%s' IS THE CURRENT PROFILE", profile_fpath)
        else:
