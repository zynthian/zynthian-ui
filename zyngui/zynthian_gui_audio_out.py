...
class zynthian_gui_audio_out(zynthian_gui_selector):
    ...
    def set_layer(self, layer):
        try:
            self.end_layer = self.zyngui.screens['layer'].get_fxchain_ends(layer)[
                0]
        except:
            self.end_layer = None
    ...
    def fill_list(self):
        ...
        if self.end_layer:
            if isinstance(self.end_layer, zynthian_gui_selector):
                port_names = [
                    "system"] + list(zynautoconnect.get_audio_input_ports(True).keys())
            else:
                port_names = zynautoconnect.get_audio_input_ports().keys()

            for k in port_names:
                try:
                    title = self.zyngui.screens['layer'].get_layer_by_jackname(
                        k).get_basepath()
                except:
                    title = k
