...
class zynthian_gui_stepsequencer(zynthian_gui_base.zynthian_gui_base):
    ...
    def __init__(self):
        ...
        # Object that currently "owns" encoder, indexed by encoder
        self.zyncoder_owner = [None, None, None, None]
        # Zyncoder step. 0 for dynamic step (speed variable).
        self.zyncoder_step = [1, 1, 1, 1]
        # Object that currently "owns" switch, indexed by (switch *3 + type)
        self.switch_owner = [None] * 12
        self.zyngui = zynthian_gui_config.zyngui  # Zynthian GUI configuration
        self.bank = 1  # Currently displayed bank of sequences
        # Root layer indexed by MIDI channel
        self.layers = [None for i in range(16)]
...
#		font=tkFont.Font(family=zynthian_gui_config.font_topbar[0], size=int(self.height * 0.05)),
        ...
        iconsize = (zynthian_gui_config.topbar_height - 4,
                    zynthian_gui_config.topbar_height - 4)
        self.image_play = ImageTk.PhotoImage(Image.open(
            "/zynthian/zynthian-ui/icons/playing.png").resize(iconsize))
        self.image_playing = ImageTk.PhotoImage(Image.open(
            "/zynthian/zynthian-ui/icons/playing.png").resize(iconsize))
        self.image_back = ImageTk.PhotoImage(Image.open(
            "/zynthian/zynthian-ui/icons/back.png").resize(iconsize))
        self.image_forward = ImageTk.PhotoImage(Image.open(
            "/zynthian/zynthian-ui/icons/tick.png").resize(iconsize))
        ...
        self.param_title_canvas = tkinter.Canvas(
            self.param_editor_canvas, height=zynthian_gui_config.topbar_height, bd=0, highlightthickness=0, bg=zynthian_gui_config.color_header_bg)
        ...
        self.listbox_text_height = tkFont.Font(
            font=zynthian_gui_config.font_listbox).metrics('linespace')
        ...
        # Pointer to instance of last child shown - used to return to same screen
        self.last_child = self.zynpad
    ...
    def populate_menu(self):
        ...
        if self.child != self.zynpad:
            self.add_menu(
                {'Pads': {'method': self.show_child, 'params': self.zynpad}})
        if self.child != self.arranger:
            self.add_menu(
                {'Arranger': {'method': self.show_child, 'params': self.arranger}})
        if self.child != self.pattern_editor:
            self.add_menu({'Bank': {'method': self.show_param_editor, 'params': {
                          'min': 1, 'max': 64, 'value': self.bank, 'on_change': self.on_menu_change}}})
        if zynthian_gui_config.enable_touch_widgets:
            self.add_menu({'Tempo': {'method': self.show_param_editor, 'params': {
                          'min': 1.0, 'max': 500.0, 'get_value': libseq.getTempo, 'on_change': self.on_menu_change}}})
        self.add_menu({'Beats per bar': {'method': self.show_param_editor, 'params': {
                      'min': 1, 'max': 64, 'get_value': libseq.getBeatsPerBar, 'on_change': self.on_menu_change}}})
    ...
    def set_title(self, title, fg=None, bg=None, timeout=None):
        ...
        if timeout:
            ...
            if bg:
                self.title_bg = bg
        self.title_canvas.itemconfig(
            "lblTitle", text=title, fill=self.title_fg)
        if fg:
    ...
    def show_menu(self):
        ...
        rows = min((self.height - zynthian_gui_config.topbar_height -
                   button_height) / self.listbox_text_height - 1, self.lst_menu.size())
    ...
    def show_param_editor(self, menu_item):
        ...
        if self.get_param(menu_item, 'get_value'):
            self.set_param(menu_item, 'value', self.get_param(
                menu_item, 'get_value')())
        self.param_editor_canvas.grid_propagate(False)
        self.param_editor_canvas.grid(column=0, row=0, sticky='nsew')
        # Get the value to display in the param editor
        self.param_title_canvas.itemconfig("lbl_param_editor_value",
                                           text=self.menu_items[menu_item]['params']['on_change'](
                                               self.menu_items[menu_item]['params'])
                                           )
        if 'on_assert' in self.menu_items[menu_item]['params']:
            self.param_editor_canvas.itemconfig(
                "btnparamEditorAssert", state='normal')
        else:
            self.param_editor_canvas.itemconfig(
                "btnparamEditorAssert", state='hidden')
        for encoder in range(4):
            self.unregister_zyncoder(encoder)
    ...
    def change_param(self, value):
        ...
        self.set_param(self.param_editor_item, 'value', value)
        result = self.get_param(self.param_editor_item, 'on_change')(
            self.menu_items[self.param_editor_item]['params'])
        if result == -1:
            self.hide_param_editor()
        else:
            self.param_title_canvas.itemconfig(
                "lbl_param_editor_value", text=result)
        ...
        if bank > 0:
            if libseq.getSequencesInBank(bank) == 0:
                ...
                for column in range(4):
                    ...
                    for row in range(4):
                        pad = row + 4 * column
                        zynseq.set_sequence_name(bank, pad, "%d" % (
                            libseq.getPatternAt(bank, pad, 0, 0)))
    ...
    def select_filename(self, filename):
        zynthian_gui_fileselector(
            self, self.load, USER_PATH, "zynseq", filename)
    ...
    def on_zyncoder(self, encoder, value):
        if self.lst_menu.winfo_viewable():
            ...
            self.set_title("Tempo: %0.1f BPM" %
                           (libseq.getTempo()), None, None, 2)
    ...
    def register_zyncoder(self, encoder, object, step=1):
        ...
        if self.shown and zyncoder:
            ...
            zyncoder.setup_zyncoder(
                encoder, pin_a, pin_b, 0, 0, None, 64, 128, step)
