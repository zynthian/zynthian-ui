...
class zynthian_gui_control_test(zynthian_gui_base):
    ...
    def on_size(self, event):
        ...
        self.test_canvas.coords(
            self.text_info, self.canvas_width // 2, self.height // 2 - text_fs * 1)
        self.test_canvas.itemconfigure(self.text_info, width=self.canvas_width, font=(
            zynthian_gui_config.font_family, text_fs))
        self.test_canvas.coords(
            self.text_error, self.canvas_width // 2, self.height // 2 + text_fs * 2)
        self.test_canvas.itemconfigure(self.text_error, width=self.canvas_width, font=(
            zynthian_gui_config.font_family, text_fs))
    ...
    def update_button_test(self):
        ...
        self.test_canvas.itemconfigure(
            self.text_info, text=self.test_button_name)
        ...
        if t == 'P':
            ...
            if self.test_button_index < len(self.zynswitch_info):
                if swi == self.zynswitch_info[self.test_button_index][1]:
                    ...
                    self.test_canvas.itemconfigure(
                        self.text_error, text="WRONG BUTTON: {}".format(swi))
            # Test LED's colors
            else:
