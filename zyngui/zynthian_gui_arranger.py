...
class zynthian_gui_arranger():
    ...
    def __init__(self, parent):
        self.parent = parent
        self.play_modes = ['Disabled', 'Oneshot', 'Loop',
        ...
        self.pattern_canvas = tkinter.Canvas(self.main_frame,
                                             ...
                                             bd=0,
                                             highlightthickness=0)
        self.pattern_canvas.create_text(self.seq_track_title_width / 2, self.timebase_track_height / 2, tags="patternIndicator", fill='white', text='%d' % (
            self.pattern), font=tkFont.Font(family=zynthian_gui_config.font_topbar[0], size=int(0.6 * self.timebase_track_height)))
    ...
    def setup_encoders(self):
        self.parent.register_zyncoder(
            zynthian_gui_stepsequencer.ENC_BACK, self)
        self.parent.register_zyncoder(
            zynthian_gui_stepsequencer.ENC_SELECT, self)
        self.parent.register_zyncoder(
            zynthian_gui_stepsequencer.ENC_LAYER, self)
        self.parent.register_switch(
            zynthian_gui_stepsequencer.ENC_SELECT, self, 'SB')
        self.parent.register_switch(
            zynthian_gui_stepsequencer.ENC_SNAPSHOT, self, 'SB')
    ...
    def populate_menu(self):
        ...
        self.parent.add_menu({'Vertical zoom': {'method': self.parent.show_param_editor, 'params': {
                             'min': 1, 'max': 64, 'value': self.vertical_zoom, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'Horizontal zoom': {'method': self.parent.show_param_editor, 'params': {
                             'min': 1, 'max': 64, 'value': self.horizontal_zoom, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'MIDI channel': {'method': self.parent.show_param_editor, 'params': {
                             'min': 1, 'max': 16, 'get_value': self.get_track_channel, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'Group': {'method': self.parent.show_param_editor, 'params': {
                             'min': 0, 'max': 25, 'get_value': self.get_group, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'Play mode': {'method': self.parent.show_param_editor, 'params': {
                             'min': 0, 'max': len(self.play_modes)-1, 'get_value': self.getMode, 'on_change': self.on_menu_change}}})
        self.parent.add_menu({'Pattern': {'method': self.parent.show_param_editor, 'params': {
                             'min': 1, 'max': 999, 'get_value': self.get_pattern, 'on_change': self.on_menu_change}}})
        ...
        self.parent.add_menu(
            {'Clear sequence': {'method': self.clear_sequence}})
    ...
    def clear_bank(self, params=None):
        self.zyngui.show_confirm("Clear all sequences from bank %d and reset to 4x4 grid of new sequences?" % (
            self.parent.bank), self.do_clear_bank)
    ...
    def clear_sequence(self, params=None):
        ...
        self.zyngui.show_confirm(
            'Clear all patterns from sequence "%s"?' % (name), self.do_clear_sequence)
    ...
    def remove_track(self, params=None):
        self.zyngui.show_confirm(
            "Remove track from sequence?", self.do_remove_track)

    # Function to actually remove selected track

    def do_remove_track(self, params=None):
        libseq.removeTrackFromSequence(
            self.parent.bank, self.sequence, self.track)
        self.update_sequence_tracks()
        self.redraw_pending = 2
    ...
    def get_smf(self, params=None):
        zynthian_gui_fileselector.zynthian_gui_fileselector(self.parent, self.import_smf, zynthian_gui_stepsequencer.os.environ.get(
            'ZYNTHIAN_MY_DATA_DIR', "/zynthian/zynthian-my-data") + "/capture", "mid", None, True)

    # Function to  check if SMF will overwrite tracks in sequence
    #	filename: Filename of SMF to import

    def import_smf(self, filename):
        logging.warning("Seq len:%d pos:%d", libseq.getSequenceLength(
            self.parent.bank, self.sequence), self.selected_cell[0])
        if libseq.getSequenceLength(self.parent.bank, self.sequence) > self.selected_cell[0] * 24:
            self.zyngui.show_confirm(
                "Import will overwrite part of existing sequence. Do you want to continue?", self.do_import_smf, filename)
        else:
...
#		libseq.clearSequence(bank, sequence) # TODO Do not clear sequence, get sequence length, start at next bar position or current cursor position
        ...
        for channel in range(16):
            ...
            note_on_info = [[None, None, None, None] for i in range(127)]
            # Position of current pattern within track in ticks
            pattern_position = self.selected_cell[0] * ticks_per_beat
                ...
                if type == 0x01:
                    ...
                    if status == note_on and velocity:
                        ...
                        if time >= pattern_position + ticks_in_pattern or pattern == None:
                            ...
                            libseq.addPattern(
                                bank, sequence, channel, position, pattern, True)
                        ...
                        libseq.addNote(step, note, velocity, 1)
    ...
    def on_time_drag_motion(self, event):
        ...
        self.col_offset = pos
        self.redraw_pending = 1
        duration = int(libseq.getPatternLength(
    ...
    def on_grid_press(self, event):
        ...
        self.pattern_to_add = libseq.getPattern(
            self.parent.bank, self.sequence, self.track, self.source_col * self.clocks_per_division)
    ...
    def on_grid_drag(self, event):
        ...
        if self.grid_timer.isAlive():
            ...
            self.pattern_to_add = libseq.getPattern(
                self.parent.bank, self.sequence, self.track, time)
            if self.pattern_to_add == -1:
                self.pattern_to_add = self.pattern
    ...
    def show_pattern_editor(self):
        ...
        time = self.selected_cell[0] * self.clocks_per_division
        pattern = libseq.getPattern(
            self.parent.bank, self.sequence, self.track, time)
        channel = libseq.getChannel(
            self.parent.bank, self.sequence, self.track)
        if pattern > 0:
            self.parent.show_child(self.parent.pattern_editor, {
    ...
    def draw_sequence_label(self, row):
        ...
        if track == 0 or row == 0:
            ...
            self.grid_canvas.create_line(0, self.row_height * (row + 1), 0, self.row_height * (
                row), fill=PLAYHEAD_CURSOR, tags=('playheadline', 'playheadline-%d' % (row)), state='hidden')
        else:
    ...
    def on_sequence_click(self, event):
        tags = self.sequence_title_canvas.gettags(
            self.sequence_title_canvas.find_withtag(tkinter.CURRENT))
        if not tags:
            return
    ...
    def draw_cell(self, col, row):
        ...
        if pattern == -1 and col == 0:
            ...
            pattern = libseq.getPatternAt(
                self.parent.bank, sequence, track, time)
            if pattern != -1:
                duration = int(libseq.getPatternLength(
        ...
        if not cell:
            ...
            cell = self.grid_canvas.create_rectangle(
                coord, fill=fill, width=0, tags=("%d,%d" % (col, row), "gridcell", "pattern"))
            celltext = self.grid_canvas.create_text(
                coord[0] + 1, coord[1] + self.row_height / 2, fill=CELL_FOREGROUND, tags=("%d,%d" % (col, row), "pattern"))
...
#				self.grid_canvas.itemconfig("lastpatterntext%d" % row, text="+%d" % (duration - libseq.getPatternLength() + time), state="normal")
    ...
    def draw_grid(self):
        ...
        for bar in range(offset, self.horizontal_zoom, libseq.getBeatsPerBar()):
            ...
            if bar:
                self.timebase_track_canvas.create_text(
                    bar * self.column_width, 0, fill='white', text="%d" % (bar+self.col_offset), anchor='n', tags='barlines')
            else:
                self.timebase_track_canvas.create_text(
                    bar * self.column_width, 0, fill='white', text="%d" % (bar+self.col_offset), anchor='nw', tags='barlines')
        ...
        for event in range(libseq.getTimebaseEvents(self.parent.bank)):
            time = libseq.getTimebaseEventTime(
                self.parent.bank, event) / self.clocks_per_division
            if time >= self.col_offset and time <= self.col_offset + self.horizontal_zoom:
                command = libseq.getTimebaseEventCommand(
                    self.parent.bank, event)
                if command == 1:  # Tempo
                    tempoX = (time - self.col_offset) * self.column_width
                    data = libseq.getTimebaseEventData(self.parent.bank, event)
                    if tempoX:
                        self.timebase_track_canvas.create_text(
                            tempoX, tempo_y, fill='red', text=data, anchor='n', tags='bpm')
                    else:
                        self.timebase_track_canvas.create_text(
                            tempoX, tempo_y, fill='red', text=data, anchor='nw', tags='bpm')
    ...
    def select_cell(self, time=None, row=None, snap=True, scroll=True):
        ...
        self.parent.set_title("Bank %d %s%d-%d (%d) %s" % (self.parent.bank, chr(65 + libseq.getGroup(self.parent.bank, sequence)), sequence+1,
                              track + 1, libseq.getChannel(self.parent.bank, sequence, track) + 1, self.get_note(libseq.getTriggerNote(self.parent.bank, sequence))))
        if not duration:
            duration = 1
        ...
        if snap:
            ...
            for previous in range(time - 1, -1, -1):
                ...
                prev_pattern = libseq.getPattern(
                    self.parent.bank, sequence, track, previous * self.clocks_per_division)
                if prev_pattern == -1:
                    continue
                prev_duration = int(libseq.getPatternLength(
            ...
            for next in range(time + 1, time + duration * 2):
                next_pattern = libseq.getPattern(
                    self.parent.bank, sequence, track, next * self.clocks_per_division)
                if next_pattern == -1:
                    continue
                next_start = next
                break
    ...
    def on_menu_change(self, params):
        ...
        if menu_item == 'Vertical zoom':
            ...
            libseq.setChannel(self.parent.bank, self.sequence,
                              self.track, value - 1)
            self.redraw_pending = 2
            try:
    ...
    def get_track_channel(self):
        channel = libseq.getChannel(
            self.parent.bank, self.sequence, self.track) + 1
        return channel
    ...
    def refresh_status(self):
        ...
        for row in range(self.vertical_zoom):
            ...
            if sequence != previous_sequence and sequence != -1:
                self.grid_canvas.coords('playheadline-%d' %
                                        seq_row, x, y1, x, y2)
                self.grid_canvas.itemconfig(
                    'playheadline-%d' % seq_row, state='normal')
            pos = libseq.getPlayPosition(
                self.parent.bank, sequence) / self.clocks_per_division
    ...
    def on_zyncoder(self, encoder, value):
        if encoder == zynthian_gui_stepsequencer.ENC_BACK:
            ...
            self.select_cell(
                self.selected_cell[0], self.selected_cell[1] + value)
        elif encoder == zynthian_gui_stepsequencer.ENC_SELECT:
            # SELECT encoder adjusts time selection
            self.select_cell(
                self.selected_cell[0] + value, self.selected_cell[1])
