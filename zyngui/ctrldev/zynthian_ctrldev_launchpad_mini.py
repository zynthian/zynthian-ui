...
class zynthian_ctrldev_launchpad_mini(zynthian_ctrldev_zynpad):
	...
	def refresh_zynpad_bank(self):
		...
		for row in range(0, 8):
			...
			if row == self.zynpad.bank - 1:
				lib_zyncore.dev_send_note_on(self.idev, 0, note, self.PLAYING_COLOUR)
			else:
				lib_zyncore.dev_send_note_on(self.idev, 0, note, self.OFF_COLOUR)
	...
	def light_off(self):
		for row in range(8):
			for col in range(9):
				note = 16 * row + col
				lib_zyncore.dev_send_note_on(self.idev, 0, note, 0xC)
