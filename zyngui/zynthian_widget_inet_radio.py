    ...
    def refresh_gui(self):
        ...
        if self.height < 400:
            ...
            if self.monitors["info"] in ["stream unavailable", "waiting for stream..."]:
                self.info_page = 1
            if self.info_page == 1:
                self.widget_canvas.itemconfigure(self.lbl_title, text=self.monitors["title"])
            elif self.info_page == 2:
                self.widget_canvas.itemconfigure(self.lbl_title, text=self.monitors["info"])
            elif self.info_page == 3:
                self.widget_canvas.itemconfigure(self.lbl_title, text=self.monitors["bitrate"])
            elif self.info_page == 4:
                self.widget_canvas.itemconfigure(self.lbl_title, text=self.monitors["audio"])
            else:
                self.widget_canvas.itemconfigure(self.lbl_title, text=self.monitors["codec"])
                self.info_page = 0
        else:
            self.widget_canvas.itemconfigure(self.lbl_title, text=self.monitors["title"])
