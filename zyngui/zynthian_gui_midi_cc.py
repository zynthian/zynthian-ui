...
class zynthian_gui_midi_cc(zynthian_gui_selector):
    ...
    def config(self, chan_from, chan_to):
        ...
        self.cc = zyncoder.lib_zyncoder.get_midi_filter_clone_cc(
            chan_from, chan_to).tolist()
    ...
    def set_clone_cc(chan_from, chan_to, cc):
        ...
        zyncoder.lib_zyncoder.set_midi_filter_clone_cc(
            chan_from, chan_to, cc_array)
        ...
        for i, ccnum in enumerate(self.cc):
            if ccnum:
                self.list_data.append(
                    (str(i), i, "[x] CC {}".format(str(i).zfill(2))))
            else:
                self.list_data.append(
                    (str(i), i, "[  ] CC {}".format(str(i).zfill(2))))
        super().fill_list()
    ...
    def select_action(self, i, t='S'):
        ...
        logging.info("MIDI CC {} CLONE CH#{}=>CH#{}: {}".format(
            cc_num, self.chan_from, self.chan_to, self.cc[cc_num]))

    def set_select_path(self):
        try:
            self.select_path.set(
                "Clone {} => {} / CC...".format(self.chan_from+1, self.chan_to+1))
        except:
