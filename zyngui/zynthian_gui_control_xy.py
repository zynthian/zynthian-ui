...
class zynthian_gui_control_xy():
    ...
    def __init__(self):
        ...
        self.hline = self.canvas.create_line(
            0, self.y, zynthian_gui_config.display_width, self.y, fill=zynthian_gui_config.color_on)
        self.vline = self.canvas.create_line(
            self.x, 0, self.x, zynthian_gui_config.display_width, fill=zynthian_gui_config.color_on)
    ...
    def get_controller_values(self):
        if self.x_zctrl.value != self.xvalue:
            ...
            if self.x_zctrl.is_logarithmic:
                self.x = int(self.width*math.log(self.xvalue /
                             self.x_zctrl.value_min)/self.x_zctrl.log_powbase)
            else:
                self.x = int(
                    self.width*(self.xvalue-self.x_zctrl.value_min)/self.x_zctrl.value_range)
        ...
        if self.y_zctrl.value != self.yvalue:
            ...
            if self.y_zctrl.is_logarithmic:
                self.y = int(self.height*math.log(self.yvalue /
                             self.y_zctrl.value_min)/self.y_zctrl.log_powbase)
            else:
                self.y = int(self.height*(self.yvalue -
                             self.y_zctrl.value_min)/self.y_zctrl.value_range)
    ...
    def refresh(self):
        ...
        if self.x_zctrl.is_logarithmic:
            xv = self.x_zctrl.value_min * \
                pow(self.x_zctrl.powbase, self.x/self.width)
        else:
        ...
        if self.y_zctrl.is_logarithmic:
            yv = self.y_zctrl.value_min * \
                pow(self.y_zctrl.powbase, self.y/self.height)
        else:
    ...
    def zyncoder_read(self):
        ...
        if self.last_motion_ts is None or (datetime.now()-self.last_motion_ts).total_seconds() > 0.1:
            if self.last_motion_ts is not None:
                self.last_motion_ts = None
                self.zyngui.screens['control'].set_controller_value(
                    self.x_zctrl)
                self.zyngui.screens['control'].set_controller_value(
                    self.y_zctrl)
