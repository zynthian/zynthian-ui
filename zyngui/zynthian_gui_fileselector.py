...
class zynthian_gui_fileselector():
    ...
    def __init__(self, parent, function, path, ext, filename=None, return_full_path=False):
        ...
        iconsize = (zynthian_gui_config.topbar_height - 2,
                    zynthian_gui_config.topbar_height - 2)
        self.imgBack = ImageTk.PhotoImage(Image.open(
        ...
        # File listbox
        self.listboxTextHeight = tkFont.Font(
        ...
        rows = min((zynthian_gui_config.display_height - zynthian_gui_config.topbar_height) /
                   self.listboxTextHeight - 1, self.file_list.size())
    ...
    def assert_selection(self):
        if self.return_full_path:
            self.function(self.files[self.file_list.get(
                self.file_list.curselection()[0])])
        else:
...
#		self.btnCancel.destroy()
    ...
    def on_switch(self, switch, type):
        if switch == ENC_SELECT and type == 'B':
            ...
            self.parent.zyngui.show_confirm("Do you really want to delete %s?" % (
                filename), self.delete_confirmed, filename)
