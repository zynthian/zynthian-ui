...
class zynthian_gui_engine(zynthian_gui_selector):
    ...
    def init_engine_info(cls):
        ...
        cls.engine_info = OrderedDict([
            ["MX", ("Mixer", "ALSA Mixer", "MIXER",
                    None, zynthian_engine_mixer, True)],
            ...
            ['CS', ("CSound", "CSound Audio Language", "Special",
                    None, zynthian_engine_csound, False)],
            ['MD', ("MOD-UI", "MOD-UI - Plugin Host", "Special",
                    None, zynthian_engine_modui, True)]
        ])
        ...
        if check_pianoteq_binary():
            ...
            cls.engine_info['PT'] = (
                PIANOTEQ_NAME, pianoteq_title, "MIDI Synth", None, zynthian_engine_pianoteq, True)

        for plugin_name, plugin_info in get_jalv_plugins().items():
            eng = 'JV/{}'.format(plugin_name)
            cls.engine_info[eng] = (plugin_name, plugin_name, plugin_info['TYPE'], plugin_info.get(
                'CLASS', None), zynthian_engine_jalv, plugin_info['ENABLED'])
        ...
        for cat, infos in sorted(self.filtered_engines_by_cat().items(), key=lambda kv: "!" if kv[0] is None else kv[0]):
            ...
            if cat:
                if self.engine_type == "MIDI Synth":
                    self.list_data.append(
                        (None, len(self.list_data), "> LV2 {}".format(cat)))
                else:
                    self.list_data.append(
            ...
            for eng, info in infos.items():
                ...
                if eng not in self.check_channels_engines or all(chan in self.zyngui.screens['layer'].get_free_midi_chans() for chan in info[4].get_needed_channels()):
                    self.list_data.append(
                        (eng, len(self.list_data), info[1], info[0]))

        # Display help if no engines are enabled ...
        if len(self.list_data) == 0:
            self.list_data.append((None, len(
                self.list_data), "Enable LV2-plugins on webconf".format(os.uname().nodename)))
        ...
        for i, val in enumerate(self.list_data):
            if val[0] == None:
                self.listbox.itemconfig(
                    i, {'bg': zynthian_gui_config.color_off, 'fg': zynthian_gui_config.color_tx_off})

    def select_action(self, i, t='S'):
        if i is not None and self.list_data[i][0]:
            self.zyngui.screens['layer'].add_layer_engine(
                self.list_data[i][0], self.midi_chan)
    ...
    def start_engine(self, eng):
        if eng not in self.zyngines:
            ...
            if eng[0:3] == "JV/":
                ...
                self.zyngines[eng] = zynthian_engine_class(
                    info[0], info[2], self.zyngui)
            else:
