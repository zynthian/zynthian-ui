...
class zynthian_gui_midi_chan(zynthian_gui_selector):
        ...
        if self.mode == 'ADD' or self.mode == 'SET':
            ...
            for i in self.chan_list:
                if i in (self.midi_chan, zynthian_gui_config.master_midi_channel):
                    ...
                    cc_to_clone = zyncoder.lib_zyncoder.get_midi_filter_clone_cc(
                        self.midi_chan, i).nonzero()[0]
                    self.list_data.append(
                        (str(i+1), i, "[x] CH#{}, CC {}".format(i+1, ' '.join(map(str, cc_to_clone)))))
                    logging.debug("CC TO CLONE: {}".format(cc_to_clone))
                else:
                    self.list_data.append(
                        (str(i+1), i, "[  ] CH#{}".format(i+1)))
        super().fill_list()
        ...
        for item in self.list_data:
            if item[2][:2] == '[x':
                self.listbox.itemconfig(
                    i, {'fg': zynthian_gui_config.color_hl})
            else:
                self.listbox.itemconfig(
    ...
    def select_action(self, i, t='S'):
        ...
        if self.mode == 'ADD':
            ...
            sublayers = self.zyngui.screens['layer'].get_fxchain_layers(
                root_layer) + self.zyngui.screens['layer'].get_midichain_layers(root_layer)
            for layer in sublayers:
                layer.set_midi_chan(selchan)
                logging.info(
                    "LAYER {} -> MIDI CHANNEL = {}".format(layer.get_path(), selchan))
                ...
                if t == 'S':
                    if zyncoder.lib_zyncoder.get_midi_filter_clone(self.midi_chan, selchan):
                        zyncoder.lib_zyncoder.set_midi_filter_clone(
                            self.midi_chan, selchan, 0)
                    else:
                        zyncoder.lib_zyncoder.set_midi_filter_clone(
                            self.midi_chan, selchan, 1)

                    self.update_list()
                    logging.info("CLONE MIDI CHANNEL {} => {}".format(
                        self.midi_chan, selchan))

                elif t == 'B':
                    self.clone_config_cc()

    def clone_config_cc(self):
        self.zyngui.screens['midi_cc'].config(
            self.midi_chan, self.midi_chan_sel)
    ...
    def set_select_path(self):
        if self.mode == 'ADD' or self.mode == 'SET':
            ...
            self.select_path.set(
                "Clone MIDI Channel {} to ...".format(self.midi_chan+1))
