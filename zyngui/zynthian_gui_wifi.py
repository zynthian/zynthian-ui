...
class zynthian_gui_wifi(zynthian_gui_selector_info):
    ...
    def build_view(self):
        if not self.scan_thread:
            self.wifi_scan = True
            self.scan_thread = Thread(target=self.scan_wifi_task, name="wifi_scan")
    ...
    def fill_list(self):
        ...
        if wifi_status_code == 20:
            self.list_data = [(self.enable_wifi, None, "\u2610 Wi-Fi is disabled", False, False, ["Toggle Wi-Fi enable.", None])]
        elif wifi_status_code in (30, 50, 100):
            self.list_data = [(self.disable_wifi, None, "\u2612 Wi-Fi is enabled", False, False, ["Toggle Wi-Fi enable.", None])]
            if not self.wifi_data:
                self.list_data.append((None, None, "Scanning Wi-Fi Networks...", False, False))
            else:
                self.list_data.append((None, None, "Wi-Fi Networks", False, False))
                self.list_data += self.wifi_data
        else:
            self.list_data = [(None, None, f"Wi-Fi doesn't work! ({wifi_status_code})", False, False)]
        super().fill_list()

    def enable_wifi(self):
        self.state_manager.start_busy("wifi_enable", f"Enabling Wi-Fi")
        try:
            check_output(["nmcli", "--terse", "radio", "wifi", "on"], encoding='utf-8')
            res = True
            sleep(2)
    ...
    def disable_wifi(self):
        ...
        try:
            check_output(["nmcli", "--terse", "radio", "wifi", "off"], encoding='utf-8')
            res = True
        except Exception as e:
            res = False
    ...
    def enable_wifi_nw(self, name):
        self.state_manager.start_busy("wifi_enable_nw", f"Connecting to {name}")
        try:
            output = check_output(["nmcli", "--terse", "con", "up", name], encoding='utf-8')
    ...
    def disable_wifi_nw(self, name):
        self.state_manager.start_busy("wifi_disable_nw", f"Disconnecting from {name}")
        try:
            output = check_output(["nmcli", "--terse", "con", "down", name], encoding='utf-8')
            if "successfully deactivated" in output:
                res = True
                self.state_manager.set_busy_success(f"Disconnected from {name}")
                sleep(2)
            else:
                res = False
                logging.debug(f"{output}")
            ...
            res = False
            self.state_manager.set_busy_error(f"Can't disable network {name}", str(e))
            sleep(3)
    ...
    def configure_wifi_nw_cb(self, passwd):
        self.state_manager.start_busy("wifi_config_nw", f"Connecting to {self.config_wifi_name}")
        try:
            output = check_output(["nmcli", "dev", "wifi", "connect", self.config_wifi_name, "password", passwd], encoding='utf-8')
            if "successfully activated" in output:
                res = True
                self.state_manager.set_busy_success(f"Connected to {self.config_wifi_name}")
                self.config_wifi_name = None
                sleep(2)
            else:
                res = False
