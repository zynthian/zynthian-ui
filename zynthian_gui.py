...
class zynthian_gui:
    ...
    def init_midi(self):
        try:
            ...
            lib_zyncoder.set_midi_filter_cc_automode(
                zynthian_gui_config.midi_cc_automode)
            # Set MIDI System Messages flag
            lib_zyncoder.set_midi_filter_system_events(
    ...
    def osc_init(self, port=1370, proto=liblo.UDP):
        try:
            ...
            self.osc_server_url = liblo.Address(
                'localhost', self.osc_server_port, proto).get_url()
            logging.info(
    ...
    def close_modal_timer(self, tms=3000):
        self.cancel_modal_timer()
        self.modal_timer_id = zynthian_gui_config.top.after(
            tms, self.close_modal)
    ...
    def show_control_xy(self, xctrl, yctrl):
        ...
        logging.debug("SHOW CONTROL-XY => %s, %s" %
                      (xctrl.symbol, yctrl.symbol))
        ...
        if self.curlayer:
            ...
            if curlayer_chan is not None and zynthian_gui_config.midi_single_active_channel:
                ...
                if cur_active_chan == active_chan:
                    ...
                    logging.debug("ACTIVE CHAN: {} => {}".format(
    ...
    def zynswitches_midi_setup(self, curlayer_chan=None):
        ...
        for i, event in enumerate(zynthian_gui_config.zynaptik_da_midi_events):
            if event is not None:
                ...
                if midi_chan is not None:
                    lib_zyncoder.setup_zynaptik_cvout(
                        i, event['type'], midi_chan, event['num'])
    ...
    def wait_threads_end(self, n=20):
        ...
        if n <= 0:
            ...
            logging.debug("Remaining {} active threads...".format(
    ...
    def init_mpe_zones(self, lower_n_chans, upper_n_chans):
        ...
        if not isinstance(lower_n_chans, int) or lower_n_chans < 0 or lower_n_chans > 0xF:
            logging.error(
                "Can't initialize MPE Lower Zone. Incorrect num of channels ({})".format(lower_n_chans))
        else:
        ...
        if not isinstance(upper_n_chans, int) or upper_n_chans < 0 or upper_n_chans > 0xF:
            logging.error(
                "Can't initialize MPE Upper Zone. Incorrect num of channels ({})".format(upper_n_chans))
        else:
