...
void Smf::enableDebug(bool bEnable) { m_bDebug = bEnable; }
...
size_t Smf::fileWriteString(const char* pString, size_t nSize, FILE* pFile) { return fwrite(pString, 1, nSize, pFile); }
    ...
    for (auto it = m_vTracks.begin(); it != m_vTracks.end(); ++it) {
        ...
        while (Event* pEvent = pTrack->getEvent(true)) {
            ...
            case EVENT_TYPE_MIDI:
                //					if(nRunningStatus != pEvent->getSubtype()) // Running status is more complex - need to consider if event from other track
                //will interfere
                {
...
double Smf::getDuration() { return m_fDuration; }
...
size_t Smf::getTracks() { return m_vTracks.size(); }
...
uint8_t Smf::getFormat() { return m_nFormat; }
...
uint16_t Smf::getTicksPerQuarterNote() { return m_nTicksPerQuarterNote; }

size_t Smf::getCurrentTrack() { return m_nCurrentTrack; }