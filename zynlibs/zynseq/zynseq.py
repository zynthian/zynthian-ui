...
class zynseq(zynthian_engine):
	...
	def __init__(self):
		try:
			...
			self.libseq.getPatternGroups.restype = ctypes.c_char_p
			self.libseq.getPatternsInTrack.restype = ctypes.c_char_p
	...
	def set_pattern_name(self, pattern, name):
		self.libseq.setPatternName(pattern, bytes(name, "utf-8"))


	def get_pattern_name(self, pattern):
		return self.libseq.getPatternName(pattern).decode("utf-8")
	...
	def get_patterns_in_track(self, bank, sequence, track):
		return self.libseq.getPatternsInTrack(bank, sequence, track).decode("utf-8")


	def get_pattern_groups(self):
		return self.libseq.getPatternGroups().decode("utf-8")
	...
	def rename_pattern_group(self, group, name):
		self.libseq.renamePatternGroup(bytes(group, "utf-8"), bytes(name, "utf-8"))


	def remove_pattern_group(self, group):
		self.libseq.removePatternGroup(bytes(group, "utf-8"))
