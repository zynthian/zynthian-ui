...
def get_config_fpath():
    fpath = os.environ.get(
        'ZYNTHIAN_CONFIG_DIR', '/zynthian/zynthian-sys/scripts')+"/zynthian_envars.sh"
    if not os.path.isfile(fpath):
        fpath = os.environ.get('ZYNTHIAN_SYS_DIR') + \
            "/scripts/zynthian_envars.sh"
...
def network_info():
    ...
    res = OrderedDict()
    res["Link-Local Name"] = [
        "{}.local".format(os.uname().nodename), "SUCCESS"]
