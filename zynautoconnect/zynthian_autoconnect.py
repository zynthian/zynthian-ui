...
def midi_autoconnect(force=False):
    ...
    if zynthian_gui_config.midi_aubionotes_enabled:
        aubio_out = jclient.get_ports(
            "aubio", is_output=True, is_physical=False, is_midi=True)
        try:
    ...
    if zynthian_gui_config.midi_touchosc_enabled:
        rtmidi_out = jclient.get_ports(
            "TouchOSC Bridge", is_output=True, is_physical=False, is_midi=True)
    ...
    for k, zyngine in zyngine_list.items():
        ...
        if zyngine.type in ("MIDI Synth", "MIDI Tool", "Special"):
            ...
            ports = jclient.get_ports(
                port_name, is_input=True, is_midi=True, is_physical=False)
            try:
            ...
            ports = jclient.get_ports(
                port_name, is_output=True, is_midi=True, is_physical=False)
            try:
                #logger.debug("Engine {}:{} found".format(zyngine.jackname,ports[0].short_name))
    ...
    for i, layer in enumerate(layers_list):
        if layer.get_midi_jackname() and layer.engine.type in ("MIDI Tool", "Special"):
            ...
            sports = jclient.get_ports(
                sport_name, is_output=True, is_midi=True, is_physical=False)
            if sports:
                ...
                for mi, dports in routed_in.items():
                    ...
                    if mi in layer.get_midi_out():
                        for dport in dports:
                            ...
                            try:
                                jclient.disconnect(
                                    zmr_out['ch{}_out'.format(layer.midi_chan)], dport)
                            except:
                                pass
                    else:
                        for dport in dports:
        ...
        if None in info['chans']:
            ...
            for ch in range(0, 16):
                try:
                    if ch in info['chans']:
                        jclient.connect(
                            zmr_out['ch{}_out'.format(ch)], info['port'])
                    else:
                        jclient.disconnect(
                            zmr_out['ch{}_out'.format(ch)], info['port'])
                except:
    ...
    for layer in zynthian_gui_config.zyngui.screens["layer"].root_layers:
        if layer.midi_chan is not None:
            zyncoder.lib_zyncoder.zmop_chan_set_flag_droppc(
                layer.midi_chan, int(layer.engine.options['drop_pc']))
    ...
    if zynthian_gui_config.midi_filter_output:
        ...
        if zynthian_gui_config.midi_rtpmidi_enabled:
            try:
                jclient.disconnect(
                    zmr_out['net_out'], "jackrtpmidid:rtpmidi_in")
            except:
                pass
...
def audio_autoconnect(force=False):
    ...
    for i, layer in enumerate(layers_list):
        ...
        layer_playback = [jn for jn in layer.get_audio_out(
        ) if jn.startswith("system:playback_")]
        nlpb = len(layer_playback)

        ports = jclient.get_ports(layer.get_audio_jackname(
        ), is_output=True, is_audio=True, is_physical=False)
        if len(ports) > 0:
            ...
            for ao in input_ports:
                ...
                if ao in layer.get_audio_out():
                    ...
                    for j in jrange:
                        try:
                            jclient.connect(
                                ports[j % np], input_ports[ao][j % nip])
                        except:
                            pass
                else:
        ...
        if layer.engine.type == "Audio Effect":
            midi_ports = jclient.get_ports(layer.get_midi_jackname(
            ), is_input=True, is_midi=True, is_physical=False)
            if len(midi_ports) > 0:
                try:
                    jclient.connect("ZynMidiRouter:ch{}_out".format(
                        layer.midi_chan), midi_ports[0])
                except:
                    pass

    headphones_out = jclient.get_ports(
        "Headphones", is_input=True, is_audio=True)
    ...
    if len(headphones_out) == 2 or not zynthian_gui_config.show_cpu_status:
        ...
        if len(headphones_out) == 2:
            ...
            headphones_conports_1 = jclient.get_all_connections(
                "Headphones:playback_1")
            headphones_conports_2 = jclient.get_all_connections(
        ...
        if not zynthian_gui_config.show_cpu_status:
            ...
            dpmeter_out = jclient.get_ports(
                "jackpeak", is_input=True, is_audio=True)
            dpmeter_conports_1 = jclient.get_all_connections(
                "jackpeak:input_a")
            dpmeter_conports_2 = jclient.get_all_connections(
                "jackpeak:input_b")
    ...
    if len(capture_ports) > 0:
        ...
        for rl in root_layers:
            ...
            for rlp in zynthian_gui_config.zyngui.screens["layer"].get_fxchain_pars(rl):
                ...
                rlp_in = jclient.get_ports(
                    rlp.get_audio_jackname(), is_input=True, is_audio=True)
                if len(rlp_in) > 0:
...
def start(rt=2):
    ...
    try:
        ...
        logger.error(
            "ZynAutoConnect ERROR: Can't connect with Jack Audio Server ({})".format(e))
